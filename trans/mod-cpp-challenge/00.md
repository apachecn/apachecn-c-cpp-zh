...
C++是一种通用编程语言，它结合了不同的范例，如面向对象、命令式、泛型和函数式编程。C++是为提高效率而设计的，是性能至关重要的应用程序的主要选择。在过去的几十年里，C++一直是工业、学术界和其他地方使用最广泛的编程语言之一。该语言由国际标准化组织(ISO)进行标准化，该组织目前正在研究该标准的下一个版本，名为 C++20，将于 2020 年完成。
