...
最后，我们需要将变量类型和新变量添加到我们的WebAssembly模块中对`update_emitter`的`Module.ccall`调用中:
...
我创建粒子系统配置工具也是别有用心的。可能你们中的一些人并不是为了学习游戏编程而阅读这本书。您可能已经购买了这本书，作为了解更多关于WebAssembly的有趣方式。编写这个工具是一种有趣的方式，可以了解更多关于 WebAssembly 模块和驱动该模块的 HTML 和 JavaScript 之间的交互。
