...
`GameObject`类不仅需要知道哪些组件与哪个`GameObject`实例一起使用，还需要知道哪些不需要某些组件，例如用于控制播放器的输入相关组件。对于太空入侵者++项目，我们可以做到这一点，并且在复杂的环境中生存，但是仅仅生存并不是目标；我们想要完全控制。
...
![](image/B14278_19_08.jpg)

按**播放**过渡到游戏画面:

![](image/B14278_19_09.jpg)
