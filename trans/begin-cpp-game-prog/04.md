...
作为最后一个抽象的例子，考虑一个复古文本冒险，玩家输入一个字母如“N”、“E”、“S”或“W”，以移动北、East、南或西。一个`switch`块可用于处理播放器的每个可能输入：
...
接下来是我们即将添加到 Timber 中的代码类型的预览！！！。我们希望跟踪分支或播放器位于树的哪一侧，因此我们将声明一个名为`side`的枚举，如下所示：
...
前面的代码测试数组位置[5]元素中的分支是否与播放器位于同一侧。
...
*   之后，我们声明了一个名为`side`的新枚举，它有三个可能的值：`LEFT`、`RIGHT`和`NONE`。这将用于描述各个分支以及播放器在代码中的几个位置的位置。
