...
部分应用程序的基本实现可以手动完成。我们可以简单地创建一个名为`increment`的 lambda，它调用通用的`add`函数，传递`1`作为第二个参数:
...
更有趣的是，更接近部分应用程序的概念，我们可以从调用方转发实例参数:
...
![](assets/8e9654cf-b698-47a3-a965-8bc798779a06.png)
