...
![](Images/9c9ab3d9-e0c4-41d0-b90c-2de3c0075dd2.png)
...
在这个食谱中，我们将实现这样一个应用程序。我们的小 grep 克隆将只从命令行接受一个模式，然后递归搜索我们在应用程序启动时所在的目录。然后，它将打印与我们的模式匹配的每个文件的名称。模式匹配将逐行应用，因此我们也可以打印文件匹配模式的确切行号。
