...
在本章中，您将学习 C++ 中类型推导的所有来龙去脉，包括 C++ 17 中的一些新增内容。这一章很重要，因为它将教会你所有的方法，编译器将试图为你自动推导类型信息。如果没有对 C++ 中类型推导的工作原理有一个坚定的理解，就有可能创建出不像预期那样工作的代码，尤其是在使用`auto`和模板编程的时候。从本章获得的知识将为您提供在自己的应用程序中适当利用类型推导的技能。
