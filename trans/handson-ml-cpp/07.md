...
我们可以用数学符号来表述分类任务。让 *X* 表示对象描述的集合， *Y* 是有限的名称或类标签的集合。有一个未知的目标函数，即映射![](assets/7ff580fd-a97c-487d-8c13-67110426f112.png)，其值仅在最终训练样本![](assets/7ae74160-7972-4143-8390-eb736f4ac57f.png)的对象上是已知的。因此，我们必须构建一个![](assets/5d9ffc6b-34bf-470b-ba48-3c0213a33ff7.png)算法，能够对一个![](assets/50bf0ac5-9854-4296-9647-78a53c2d1360.png)任意对象进行分类。在数理统计中，分类问题也被称为判别分析问题。
...
![](assets/c0e8d96d-ad16-44e5-91c8-397541b0cf60.png)
...
线性分类器的基本思想是特征空间可以被超平面分成两个半空间，在每个半空间中预测目标类的两个值之一。如果我们能无误差地划分一个特征空间，那么训练集就叫做**线性可分**。逻辑回归是一种独特的线性分类器，但它能够预测![](assets/402d21db-8911-4197-a6f7-7fa8c9bb810a.png)的概率，将![](assets/a9bd882c-b7e6-4d59-8a68-dce9c5759dcd.png)的例子归因于类 **+** ，如下所示:

![](assets/07e59415-5875-40be-903a-fc10fbd7d54a.png)

考虑二进制分类的任务，目标类的标签用 **+1** (正例)和 **-1** (负例)表示。我们要预测![](assets/df34c5ee-23ec-480b-90f8-ffa37cf324f6.png)的概率；因此，目前，我们可以使用以下优化技术构建线性预测:![](assets/41d81bf4-2432-4c5c-931e-ef7a6d278d8a.png)。那么，我们如何将结果值转换成极限为[0，1]的概率呢？这种方法需要特定的功能。在逻辑回归模型中，具体函数![](assets/b8bfcff5-0451-4cb2-b209-1c1ac9b2d3b7.png)用于此。

让我们用事件发生的概率 *X* 来表示 *P(X)* 。概率优势比 *OR(X)* 由![](assets/8abac566-bbf6-4621-8215-704442e1d809.png)确定。这是事件发生或不发生的概率比。我们可以看到概率和优势比都包含相同的信息。但是 *P(X)* 在 0 到 1 的范围内，而 *OR(X)* 在 0 到![](assets/f6e23ef0-ef45-49e5-9db8-3f735232c564.png)的范围内。如果你计算 *OR(X)* 的对数(称为**几率的对数**，或者**概率比的对数**，很容易看出以下适用:![](assets/422748e7-e992-4861-89b7-5da73c23cf43.png)。

利用逻辑函数预测![](assets/135c458c-858d-45a1-b330-4f7561467e4a.png)的概率，可以从概率比(暂时假设我们也有权重)得到如下:

![](assets/c5037394-aca4-43c0-84c7-4dd5555983aa.png)

因此，逻辑回归预测将样本分类到 **+** 类的概率，作为模型权重向量以及样本特征向量的线性组合的 sigmoid 变换，如下所示:

![](assets/84fcd5ec-7611-41bc-94b9-82b1c0209a26.png)

从极大似然原理出发，我们可以得到一个逻辑回归求解的优化问题——即逻辑损失函数的最小化。对于 **-** 类，概率由类似的公式确定，如下图所示:

![](assets/603ad19c-9259-4ad1-ba19-38bec195d8ac.png)

两个类的表达式可以组合成一个，如下图所示:

![](assets/b7ee7e8b-c85a-4751-a691-775ad7ff85fc.png)

在这里， <sub>![](assets/854af046-9a1f-4aa9-a4be-1fc209b1c42b.png)</sub> 这个表达被称为 <sub>![](assets/0aa64f17-e7fb-46d7-9b6f-cefb5e2b9607.png)</sub> 对象的分类边缘。分类余量可以理解为模型对对象分类的*信心*。对这一差额的解释如下:
...
在离散情况下，似然函数![](assets/8f4fc5f6-d56a-46f7-bdee-7bf94499c4d7.png)可以解释为样本 *X <sub>1</sub> 的概率。。。，* *X <sub> n </sub>* 等于 *x <sub>1</sub> 。。。，* *x <sub> n </sub>* 在给定的一组实验中。此外，该概率取决于 *** θ *** ，如下图所示:

![](assets/3dce842f-d739-46ac-80d6-80d0c5a6849e.png)

未知参数![](assets/7c394c37-bdc1-4e0f-8beb-b2721acabc8d.png)的最大似然估计![](assets/6a21805d-d705-48fc-97d6-8642c6a12775.png)称为![](assets/45627d3f-8ef8-4583-88b7-80ed4a8640d1.png)的值，对于该值，函数![](assets/5ad25318-6c27-4299-8ec1-d2435e0d1cf3.png)达到其最大值(作为 *** θ *** 的函数，具有固定的![](assets/745dba19-ffe1-43b5-a725-54163fac50bf.png))，如下图所示:

![](assets/46433970-df84-4372-9e80-4678f69cb132.png)

现在，我们可以写出样本的可能性——即观察样本![](assets/814390c7-11e4-4fc7-996e-164345001e90.png)中给定向量![](assets/9cb386ac-9b50-4975-86a8-1846ffaae5c8.png)的概率。我们做一个假设——物体独立于单一分布产生，如下图所示:

![](assets/83cd3171-cf20-426c-8cbd-a851b9572e79.png)

让我们取这个表达式的对数，因为和比乘积更容易优化，如下所示:

![](assets/ba6bb017-ba98-4f9b-acd5-8b0e402a7cb5.png)

在这种情况下，最大化可能性的原则导致表达式的最小化，如下所示:

![](assets/1e8ca68a-04c5-4a2e-b3f8-66d4ab8b71cb.png)

这个公式是一个逻辑损失函数，对训练样本的所有对象求和。通常，在模型中添加一些正则化来处理过拟合是一个好主意。逻辑回归的 L2 正则化的排列方式与线性回归(岭回归)非常相似。然而，通常使用 SVM 模型中使用的受控变量衰减参数 *C* ，其中它表示软裕度参数表示。所以，对于逻辑回归， *C* 等于逆正则化系数![](assets/01baf47c-5570-4b8d-bf4d-769b18092744.png)。 *C* 和![](assets/2c7a3c90-9656-4c97-b9dc-5ed81156461d.png)的关系如下:降低 *C* 会加强正则化效果。因此，以下功能应最小化，而不是功能性的 <sub>![](assets/e09cd379-e060-466f-a5ba-b5b939497093.png)</sub> :

![](assets/cf37a55d-db17-46e0-84bf-7d0e560a8c0c.png)
...
![](assets/2048455d-b621-425e-9cad-e693bc45c809.png)
...
考虑由带有![](assets/5cb871ef-c72f-45fd-b782-b632390e15a0.png)参数的![](assets/29ac2849-95d2-452d-8981-2e2d9776fa1e.png)对象组成的集合![](assets/5e14aa29-d31e-47c6-a876-f2f57bead3e4.png)给出的学习样本，其中![](assets/db5e59ea-7ed7-494d-a706-e1b3aac14364.png)取值-1 或 1，因此定义点的类别。每个点![](assets/d14913c4-7526-401c-843b-64d30f0eb89c.png)都是维度![](assets/39545b26-d6b3-498a-9b5b-ee3294d0411d.png)的向量。我们的任务是找到分隔观测值的最大边缘超平面。我们可以使用解析几何将任何超平面定义为满足条件的一组点![](assets/1116c0b7-3372-4a89-9607-a3ef00270fd8.png)，如下图所示:

![](assets/7c7c6e25-e206-468a-9450-42e10b3c63fc.png)

在这里，<sub>![](assets/83371f98-b206-4d93-995c-50078ae0c456.png)</sub><sub>![](assets/6c9ba1c6-87b6-445c-96c3-687e90b69ce5.png)</sub>。

因此，线性分离(鉴别)函数由方程 *g(x)=0* 描述。从点到分离函数 *g(x)=0* (从点到平面的距离)的距离等于以下值:

![](assets/e9278155-44d3-40b0-b4c1-c67237b67987.png)

![](assets/643222ab-8ac5-4ae4-a461-7e8d9d62cddb.png)在于边界的闭合即 <sub>![](assets/494a0506-2402-455f-94fa-df45c8c2e51a.png)</sub> 。边界，即分隔条的宽度，需要尽可能大。考虑到边界的闭合满足条件 <sub>![](assets/bfa8ae2a-283a-47ae-a4e9-95d15f648a0b.png)</sub> ，那么与 <sub>![](assets/963a6f7a-e5dc-44f0-8617-ba60572fc9a0.png)</sub> 的距离如下:

![](assets/09edd0f5-1e64-4378-bafd-47a5b4767bc0.png)

因此，分隔条的宽度为![](assets/cfa69e7d-1642-4b46-ac33-61c42259d629.png)。要从分割线中排除点，我们可以写出以下条件:

![](assets/994eb337-dbf8-44f9-bbad-67be48d8207d.png)

我们也来介绍一下显示![](assets/bfe3c595-0e62-422d-b3a7-182430607097.png)属于哪个类的索引函数![](assets/324aaf74-8f94-450e-a117-337b938dad7a.png)，如下:

![](assets/efded3ca-aa7f-4bd0-928e-3f2abf9847d7.png)

因此，选择产生最大宽度走廊的分离函数的任务可以写成如下:

![](assets/b4a55e6f-0e3d-461e-81f0-140ce2852307.png)

*J(w)* 功能的引入是假设![](assets/c88fff0a-e8c6-4f70-918f-77a7077edfd2.png)适用于所有![](assets/b8fcf54b-2292-4c45-8685-b89bfdb1f4ff.png)。由于目标函数是二次的，这个问题有唯一的解。

根据库恩-塔克定理，这个条件等价于以下问题:

![](assets/1cfaeb47-c40d-4538-8af5-5a27bf874fc6.png)

前提是![](assets/9a6bd0e5-737f-41c3-acc8-2847fc229db6.png)和![](assets/24c8a1f4-60bb-4a52-b182-0c02264841b3.png)，其中![](assets/c749090c-b99e-4728-b396-271008688ae6.png)是新变量。我们可以将 <sub>![](assets/03d3dd9f-4ec5-4a97-84d5-7da177e928c2.png)</sub> 改写成矩阵形式，如下:

![](assets/6218f944-1ead-42a7-b85e-a5ee3e89131a.png)

矩阵的 *H* 系数可以计算如下:![](assets/e7da0116-29d0-4eee-ae94-3f77d61089c4.png) $。二次规划方法可以解决 <sub>![](assets/65684c5f-ff7c-4593-af9c-ec34ddeb6efd.png)</sub> 任务。

在为每个![](assets/50b063aa-eb33-416d-aafc-75eb858086cc.png)找到最佳![](assets/db61aa7a-ff86-474a-8467-8cdeba9a2729.png)后，满足以下两个条件之一:

1.  ![](assets/9d3acb89-ca4c-49a6-8151-0530d090e6ab.png) ( *i* 对应于非支持向量。)
2.  ![](assets/7d0d7dd5-f443-42f8-bc37-3d730d320c2b.png) ( *i* 对应一个支持向量。)

然后，从关系![](assets/6608624f-e601-47f4-bd61-215e9e0511b5.png)中可以找到![](assets/91ae7369-da69-43dd-9d08-1538baafab00.png)，考虑到任意一个![](assets/fd8d1542-beb0-45a8-bd62-11c39c3c68ba.png)和![](assets/1e4c5a7c-959c-414d-8856-ef4f34aeffb1.png)，可以确定![](assets/f0839c74-0d61-4ca1-ada0-f0ece08a8678.png)的值，如下:

![](assets/59a1e83b-1daa-44a1-af5b-16b20535e808.png)

最后，我们可以得到判别函数，如下所示:

![](assets/c30b7e23-7783-4e23-a2ae-903f0adc8eed.png)

注意，求和不是在所有向量上进行的，而是只在集合***【S】***上进行的，这是支持向量的集合 <sub>![](assets/7c23e379-5cab-450e-b1ca-f9093abe1d37.png)</sub> 。

不幸的是，所描述的算法仅针对线性可分离的数据集实现，这本身很少发生。有两种处理线性不可分离数据的方法。

其中之一称为软边界，它选择一个超平面，尽可能纯粹地(以最小的误差)划分训练样本，同时最大化到训练数据集中最近点的距离。为此，我们必须引入额外的变量![](assets/63d92f38-fb0e-4e01-9ad9-6b717e534355.png)，这些变量表征了每个物体上的误差大小*x<sub>I</sub>T4。此外，我们可以将总误差的惩罚引入目标函数，如下所示:*

![](assets/eb0e9a2f-2e21-47fd-ad60-c15ff71a5fd2.png)

这里，![](assets/c0e2ac85-e677-45e3-b89a-9b6abf7b18d8.png)是一个方法调整参数，允许您调整最大化分割条宽度和最小化总误差之间的关系。对应对象*x<sub>I</sub>T5】的罚值![](assets/ce39c00c-aed4-47a1-95da-38e968184b36.png)取决于对象 *x <sub> i </sub>* 相对于分界线的位置。所以，如果 *x <sub> i </sub>* 位于判别函数的对面，那么我们可以假设罚值![](assets/87098cb2-bd7a-45a9-ba8d-407b52457e40.png)，如果 *x <sub> i </sub>* 位于分割线，并且来自它的类。因此，相应的权重为![](assets/5b87b854-d1d0-4823-b4a7-fae06ab6f693.png)。在理想情况下，我们假设![](assets/fb48d981-600f-456c-b16f-16a340328675.png)。由此产生的问题可以改写如下:*

![](assets/451bff32-3719-4920-a4d4-6d64bf350e63.png)

请注意，非理想情况下的元素也包含在最小化过程中，如下图所示:

![](assets/af522676-ee89-4792-8880-0c007cb617d9.png)

这里，常数***【β】***是考虑到条带宽度的重量。如果***【β】***很小，那么我们可以允许算法在非理想位置(在分割条中)定位相对较多的元素。如果 ***β*** 很大，那么我们要求在非理想位置(分割线)存在少量元素。不幸的是，由于 <sub>![](assets/10d560e8-a268-431a-91ad-3062d3225c29.png)</sub> 的不连续性，最小化问题相当复杂。相反，我们可以使用以下最小化:

![](assets/5c637e72-8854-4000-b9de-d4a28a4cd42f.png)

这在限制条件![](assets/fea3d5e3-e889-4722-be90-14deb8eb9a41.png)下发生，如下图所示:

![](assets/f4515378-8d55-4e17-a6da-91866681e389.png)
...
首先，选择映射![](assets/461c32a6-ec54-4824-b006-43426085142d.png)的功能，将![](assets/9881246d-55f3-4d82-8935-d09ba5af9eca.png)的数据映射到更高维度的空间。然后，一个非线性判别函数可以写成![](assets/b9178452-8150-4c09-9e01-270669c390af.png)的形式。该方法的思想是找到核函数 <sub>![](assets/e08de378-42a8-46a8-af6d-3b266b9a5e2c.png)</sub> 并最大化目标函数，如下图所示:

![](assets/493c5521-1089-497c-b633-b2251afbdaa1.png)
...
*   <sub>![](assets/b0f143ee-bdbc-4100-907d-5d09be95a4f4.png)</sub>
*   <sub>![](assets/9014658d-1aa3-4b69-9bc3-5af7836ff624.png)</sub> 只当 *x = y*
*   <sub>![](assets/449f7f79-ac30-41f0-8534-1b127b4040a6.png)</sub>
*   <sub>![](assets/27c274cc-c04f-483e-8169-7d1c2d75f2bb.png)</sub> 在点 *x，y，z* 的情况下不要躺在一条直线上

在这种情况下， *x，y，z* 是比较对象的特征向量。对于有序属性值，可以应用欧几里德距离，如下所示:

![](assets/ca165513-e931-4a51-80e6-67b0f83a416a.png)

在这种情况下， ***n*** 就是属性的个数。

对于无法排序的字符串变量，可以应用差分函数，其设置如下:

![](assets/2ddd3c72-8eef-4d99-9f06-6d82c30b7725.png)

在计算距离时，有时会考虑属性的重要性。通常，属性相关性可以由专家或分析师主观确定，并基于他们自己的经验、专业知识和问题解释。在这种情况下，每个 *i <sup>th</sup>* 的平方差之和乘以系数 *Z <sub>i</sub>* 。例如，如果属性 *A* 比属性 <sub>![](assets/2262cd94-92e4-4de0-8d7a-1f26273a4b77.png)</sub> ( <sub>![](assets/23695636-9293-4313-9e22-284bf14e187c.png)</sub> 、 <sub>![](assets/b6243969-9dbd-42fe-baa1-6ff9b040b778.png)</sub> )重要三倍，则距离计算如下:

![](assets/ba367d74-4d19-4fd0-831a-3c209fa07102.png)
...
![](assets/fb27dc56-4a1f-47f2-a8fc-085900ecb45a.png)

在这种情况下，![](assets/74e9ed3b-18f0-40f3-9eb7-69cc4e38a17e.png)是从已知物体![](assets/e88ee899-5081-4968-aa5a-cb732806c198.png)到新物体![](assets/5855f10c-a9bd-44bb-9ba5-b2d76c15729d.png)的距离的平方，而![](assets/652b7372-8143-4bf0-9d0c-e0b486b0dbe6.png)是为其计算选票的类别的已知物体的数量。`class`是班级的名字。新对象对应票数最多的类。在这种情况下，几个班级获得相同票数的概率要低得多。当![](assets/bf18ac93-08e7-4306-88ad-3380966d5871.png)时，新对象被分配到最近邻的类。
...
kNN 是最简单的分类算法之一，因此它在现实任务中通常是无效的。KNN 算法有几个缺点。当我们没有足够的样本时，除了分类精度低之外，kNN 分类器的问题是分类速度:如果训练集中有 *N* 个对象，并且空间维数为 K，那么对测试样本进行分类的操作次数可以估计为 <sub>![](assets/edf38951-f863-4fc4-a990-0a3a5734f294.png)</sub> 。用于算法的数据集必须具有代表性。模型不能*从数据中分离出来*:要对一个新的例子进行分类，需要用到所有的例子。

积极的特征包括算法抵抗异常异常值的事实，因为这种记录落入 kNN 数量的概率很小。如果发生这种情况，那么 <sub>![](assets/b857e75e-ff7a-47a8-a6f6-6d0800cbd893.png)</sub> 对投票的影响(唯一加权)也很可能是微不足道的，因此，对分类结果的影响也很小。该算法的程序实现相对简单，算法结果易于解释。因此，适用领域的专家在寻找相似对象的基础上理解算法的逻辑。通过使用最合适的组合函数和度量来修改算法的能力允许您针对特定任务调整算法。
...
**各对各**策略分配 <sub>![](assets/da8238b4-3088-4ee4-a9f4-905b8f59a544.png)</sub> 量词。这些分类器被训练来区分所有可能的类别对。对于输入向量，每个分类器给出 <sub>![](assets/97ecb4ab-53bc-4383-a606-ca9ee824b0e9.png)</sub> 的估计，反映类![](assets/48db792a-7a94-4907-be42-eedf991eb47b.png)和![](assets/b7a738e1-1798-486a-8def-89f379ac2078.png)中的成员关系。结果是一个具有最大和的类![](assets/8bcef908-5f4b-471a-98f9-f25a0b72b2f9.png)，其中 *g* 是一个单调的非递减函数——例如，相同或逻辑。

**射击锦标赛**策略还包括训练![](assets/f0851e2a-7892-473a-b08e-0a398e219714.png)分类器来区分所有可能的类别对。与之前的策略不同，在矢量 *X* 的分类阶段，我们安排了一个班间比武。我们创建一个锦标赛树，其中每个职业都有一个对手，只有一个获胜者可以进入下一个锦标赛阶段。因此，在每一步，只有一个分类器确定向量 *X* 类，然后*获胜的*类被用来确定具有下一对类的下一个分类器。这个过程一直进行到只剩下一个优胜班，这应该算是结果。
...
![](assets/2a56a602-6e95-4c6b-8cd3-ba3fb0843592.png)
...
![](assets/a18673d2-b79e-4def-b4ff-2922391ffbae.png)
...
![](assets/eeb5ae70-9c92-48a8-b5ce-42be109af052.png)
...
![](assets/c1eeee39-fc0f-4cb4-8ccd-5742a387afcf.png)
...
![](assets/bf565b1e-d644-4d97-b037-2e375ca8a743.png)
...
![](assets/cb66d043-b7f4-4a64-a325-bfd1514d2429.png)

您可以看到，SVM 算法的 Dlib 实现也在所有数据集上进行了正确的分类，而无需额外的配置。请记住，在某些情况下，相同算法的幕府实现进行了不正确的分类。

# 使用鲨鱼毫升库

`Shark-ML`库对于多类分类问题有一个更低级的方法。逻辑回归和 SVM 分类器被实现为二元分类器。因此，用户必须明确地训练![](assets/4ae69608-2bc4-406b-8cfc-4ad808b97f16.png)分类器并配置`OneVersusOneClassifier`类的对象，以将它们组合成多类分类器。kNN 算法本质上是一个多类分类器。
...
![](assets/7bb708d5-2a0a-4c7e-a271-669e6f60a62d.png)
...
![](assets/0bac7bf3-20f6-4e06-9346-b848d906b529.png)
...
![](assets/cb0fd3aa-2c6f-486b-b812-55bfec480210.png)
