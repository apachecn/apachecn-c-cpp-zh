...
![](assets/f5e98ccc-7ec0-4805-9a52-8fb69f22674b.png)

在上图中，神经网络的一个片段，![](assets/179b85fa-5727-40d6-924a-41feb7894736.png)(具有乙状结肠激活功能的一层神经元)，取输入值， <sub>![](assets/c6044d6d-6a22-4c17-88c6-01b54c9b24d3.png)</sub> ，并返回值， <sub>![](assets/d32d3ac7-2fc6-458c-a8cf-e1a9dbd8aaa7.png)</sub> 。反馈的存在允许我们将信息从网络的一个时间步长传递到另一个时间步长。一个循环网络可以被认为是同一个网络的几个副本，每个副本都将信息传输到下一个副本。以下是我们扩展反馈时发生的情况:

![](assets/d97991f8-0093-4a90-abf4-8428c6284491.png)

这可以更详细地表示如下:

![](assets/c68769ba-0594-4ad0-8941-cef9c005721a.png)

在这里，我们可以看到输入的数据向量，![](assets/3828550d-2809-4e4a-a88c-353f365453ed.png)。每一步的每个向量都有一个隐藏的状态向量![](assets/3de8eaa8-d807-4327-b440-63bb3942f49d.png)。我们称这种配对为**模块**。每个 RNN 模块中的隐藏状态是上一步的输入向量和隐藏状态向量的函数，如下所示:

![](assets/ccf7c8b1-3eff-47ec-bf6f-2a3522fc22de.png)

如果我们看上标，可以看到有一个权重矩阵， <sub>![](assets/4403c3cf-59fe-4d4a-8c8d-77a45efeb86f.png)</sub> ，我们乘以输入值，还有一个递归的权重矩阵， <sub>![](assets/a2bd2484-14dd-4696-9eda-9d2f061e5f59.png)</sub> ，乘以上一步的隐藏状态向量。这些递归权重矩阵在每一步都是相同的。这个概念是 RNN 的关键组成部分。如果你仔细考虑一下，这种方法与传统的两层神经网络有很大的不同。在这种情况下，我们通常为每一层选择一个单独的矩阵*W*:*W*<sub>1</sub>和 *W* <sub>2</sub> 。这里，权重的递归矩阵对于整个网络是相同的。![](assets/792fcda7-c976-4add-83f3-fd36d677c0e0.png)表示具有 sigmoid 作为激活函数的神经网络层。

此外，另一个权重矩阵 <sub>![](assets/b36b6155-961f-471f-888b-520e2837ce23.png)</sub> 用于获得每个模块的输出值 *y* ，乘以 *h* :

![](assets/4d9abf37-e671-44ef-a62b-8727bb2fa652.png)
...
BPTT 背后的想法很简单——我们展开一个循环网络一定数量的时间步长，它转换成一个通常的深度神经网络，然后用通常的反向传播方法进行训练。请注意，该方法假设我们对所有时间步长使用相同的参数。此外，当误差在状态(步骤)中向后传播时，权重梯度被相互总结。它们在网络的初始配置期间被复制![](assets/f033007b-bdf8-4b35-9783-6bf29b5a8665.png)次，就像给常规前馈网络添加层一样。展开 RNN 所需的步数对应于输入序列的长度。如果输入序列很长，那么训练网络的计算成本就会增加。

下图显示了 BPTT 方法的基本原理:

![](assets/ff071350-c4a3-40fb-a622-94222b1af0e0.png)
...
![](assets/3a6d1ea0-41a6-4488-8e18-3881d573540d.png)
...
![](assets/5cb8ae24-5fd1-48c1-a60a-c6a891f6c7ff.png)
...
![](assets/7c162192-b24f-49ec-9b1a-2335256f3f19.png)

LSTM 中的下一个流阶段决定应该在单元状态中存储什么新信息。这一阶段由两部分组成。首先，一个叫做**输入层门**的乙状线层决定了哪些值应该更新。然后， *tanh* 层构建一个新候选值的向量，![](assets/c7d72391-3822-46aa-8a8a-dd592fac526e.png)，它可以添加到单元的状态中:

![](assets/1d2290fc-d9ea-4eef-beae-9192bda2b392.png)

要用新状态替换细胞的旧状态，我们需要将 <sub>![](assets/bb353087-6e47-453d-a87c-fc4b54a97002.png)</sub> 的旧状态乘以 <sub>![](assets/2ab46549-de67-4adf-9f5a-68dc51d9111e.png)</sub> ，忘记我们决定忘记的。然后，我们添加 <sub>![](assets/218e7dc4-aba3-40b3-97a6-8b9e57a077ab.png)</sub> (新的候选值，乘以我们想要更新每个状态值的数量):

![](assets/1d6e81d3-cb80-4be1-8f21-5921ee22a99b.png)
...
![](assets/dc81fd8a-8a2b-4bf7-8a04-4b798bebe3da.png)
...
![](assets/b0dad6e1-c5e0-4adc-a559-78796e1fc0fd.png)

更新后的门决定了从最后一个状态应该保留多少信息，以及应该从前一层获取多少信息:

![](assets/2bb69294-d50b-4db4-98d4-5a8f488d34d6.png)

复位门的工作方式类似于遗忘门:

![](assets/c0d8ba47-5710-4b99-a99b-7e287265b286.png)

*tanh* 层构建了一个新的候选值向量![](assets/11714aec-9295-43c0-8632-5a3d1dd89ed3.png)，可以将其添加到单元的状态中。*复位门*的值应用于先前状态的值:

![](assets/ce4e0914-e4b6-4b5b-8896-83aa16d2be5e.png)

基于先前状态值和新候选值的组合获得新状态。*更新门*值控制应使用何种比例的状态值:

![](assets/e7a4d7a2-2827-4aeb-916a-9f3d7cb98678.png)
...
![](assets/76f2aeaa-e1c9-45fa-b08a-3718b6ffe62a.png)
...
![](assets/9efb8524-5355-4c1f-9e27-49db5b4f92ce.png)

上图显示了一个多层单向 RNN，其中层号表示为上标。另外，注意每一层都需要自己的初始隐藏状态![](assets/7a7f57c0-068f-4011-9e4a-90d08053e5ce.png)。
...
让我们将同现矩阵表示为 *X* 。在这种情况下 <sub>![](assets/46234cc2-64e9-4dcc-a556-499e523e4453.png)</sub> 指的是 *X* 中的元素 *i* 和 *j* ，等于 *j* 这个词在![](assets/3703f18f-a16d-46ed-8812-1a38d0dbdc72.png)这个词的上下文中出现的次数。我们也可以将 <sub>![](assets/ea656830-2d48-4726-b7c8-3a4f15f6258b.png)</sub> 定义为 *i* 这个词的上下文中出现的总字数。

接下来，我们需要使用单词向量生成一个表达式来估计共现率。为此，我们从以下关系开始:

![](assets/8a89f02b-32f2-4443-828e-1d9cd3b60e41.png)

这里， <sub>![](assets/b6fb666a-fc18-454d-8517-cc83a2844887.png)</sub> 表示单词 *j* 在单词 *i* 上下文中出现的概率，可以表示如下:

![](assets/8068e90a-2952-417c-ab49-c1c475e41da6.png)

*F* 是一个未知函数，对 *i* 、 *k* 和 *j* 进行嵌入。注意嵌入有两种:上下文输入和目标词输出(分别表示为 <sub>![](assets/b6bf0baa-da29-47bd-af67-8f5af02f42e7.png)</sub> 和 <sub>![](assets/02b3679f-64c8-489f-b09d-54b11710af7c.png)</sub> )。这两种嵌入是一个小细节，但仍然需要记住。
...
![](assets/1f7f6d6e-c13f-4573-b16c-506f54cb3f8a.png)

要指定 <sub>![](assets/b734bc32-6006-4334-a7e5-ae7ab25e4b14.png)</sub> 和 <sub>![](assets/72ed3e00-9365-4676-b4e7-159481621f85.png)</sub> 之间的线性关系，我们使用点积运算:

![](assets/4a3476b4-84e3-4e3e-a12c-0f65ef4cfcd8.png)
...
![](assets/9189e521-3db4-470b-beae-717a6ed15860.png)

我们可以将这个等式转换为来自同现矩阵的单个记录的等式:

![](assets/a5dc5067-60f0-4edd-bf88-6af51b8e1e8d.png)

通过这样做，我们将等式右侧的最后一项转化为偏置项。通过增加对称性的输出偏置，我们得到以下公式:

![](assets/51bd86a4-4163-45f6-871d-a21716858604.png)
...
![](assets/3bfb6585-4419-4267-a7c2-e1a865e48801.png)

使用这个函数，我们可以将损失函数转换为以下形式:

![](assets/cc76aec2-5da0-4728-9e16-7031bef564eb.png)
...
在`flat_weights`方法中，我们按照 <sub>![](assets/b4d44e09-d9be-424e-9e72-60db29a36a72.png)</sub> 的顺序将所有权重组织成一个平面向量，对每一层重复，并且彼此相邻。这是来自`torch::nn::LSTM`模块的相同方法的副本。
