...
![](assets/26139e4d-6893-44ea-8fd8-a999d35ffe71.png)

在下图中，我们可以看到图形中的异常；这些异常可以是边也可以是顶点(请参见用浅色标记的元素):

![](assets/f5cfa63c-bbd7-4cb5-8242-9c6bf2d60624.png)
...
![](assets/696b4669-7bf8-4aa4-9aac-c50376e8fadf.png)

这里， <sub>![](assets/e1508df7-b487-4538-a4b5-bb3e7f3d6b05.png)</sub> 是来自数据集的样本，是来自数据集的所有样本的均值， <sub>![](assets/9aeacf29-ae41-4ae1-82a3-348243a58cfa.png)</sub> 是数据集内样本的标准差。*Z*-值被放置在区间[-1，1]中，接近零的最小值是最正常或预期的值。下图显示了通过使用 *Z* 分数测试，某些类型的正态分布数据中的哪些值可以被视为异常值或异常值:

![](assets/bd9c38aa-46bc-4a0b-9764-2492518fa162.png)
...
![](assets/6ec20f8b-5a5b-43c9-bd71-63c3bb720425.png)

这里，![](assets/8883a333-8655-4905-83e7-685220195aa0.png)是观察的路径长度，![](assets/28f5d105-7522-4b84-b6fe-4cd7baa84623.png)，![](assets/041fc0ff-7f36-4cfd-9ace-d0a42dbda8d6.png)是来自隔离树集合的![](assets/3355ba6f-bf27-4d7f-90bb-342e59a48fcb.png)的平均值，![](assets/8892a1ae-c0f8-437b-a8ea-a391215d6667.png)是在二叉查找树搜索不成功的平均路径长度，![](assets/d3dbe3a1-550d-401a-993f-4fa12d8b2fb4.png)是外部节点的数量。
...
![](assets/ccc25f6d-0e24-44aa-ad4c-abd40ce8ec00.png)

# 用一类 SVM 检测异常

支持向量方法是一种基于使用超平面将对象划分为类的二元分类方法。超平面的尺寸总是被选择成小于原始空间的尺寸。例如在 <sub>![](assets/1e882ff0-a3f3-4e77-9c6b-5c14d6d3a29f.png)</sub> 中，超平面就是一个普通的二维平面。从超平面到每个类的距离应该尽可能短。最接近分离超平面的向量称为支持向量。实际上，数据可以被超平面分割的情况——换句话说，线性情况——非常罕见。在这种情况下，使用特殊的映射将训练数据集的所有元素嵌入到更高维度的空间![](assets/914cf5ae-a92f-495c-bf02-a7e48e447d27.png)中。在这种情况下，选择映射使得在新空间![](assets/6ac427fa-be34-48f6-95b5-4c84b5166c46.png)中，数据集是线性可分的。
...
让我们假设我们在一个数据集中有一些样本![](assets/e963c7c7-6d78-474c-bfe0-714b8bdcdafc.png)，并且它们被标记并且正态分布(高斯分布)。在这种情况下，我们可以使用分布属性来检测异常。假设函数![](assets/4a24ccc3-3875-4b16-8fd8-2d84d3960a13.png)给出了样本正常的概率。高概率对应常规样本，低概率对应异常。因此，我们可以使用以下**异常模型**公式选择阈值来区分规则值和异常:

![](assets/af954e1c-10db-44e4-9c64-77c609a98884.png)

如果[ ![](assets/5df8225b-176a-4e8c-b8a8-a634e8ed4c83.png) ]和![](assets/c570121a-eac9-4101-b08e-e75794a8908d.png)遵循均值为![](assets/e72c1ddf-73d6-4a98-91b4-2cd0de1bb265.png)，方差为![](assets/7f5bec59-bddf-4981-959a-dc7580f4dc00.png)的高斯分布，则表示如下:

![](assets/bcc15ac8-39d1-4f4e-aa8e-35873c1c6854.png)

以下公式给出了![](assets/9a191f0f-759f-4e07-b9f1-62b0ecddadfc.png)在高斯分布中的概率:

![](assets/b6a5ebc4-e29b-4579-89c4-0a257c95c609.png)

这里， <sub xmlns:epub="http://www.idpf.org/2007/ops">![](assets/6ddf599a-0d46-4fa7-a872-49f2fc6e3db4.png)</sub> 是均值， <sub xmlns:epub="http://www.idpf.org/2007/ops">![](assets/ce42fc89-7b79-475e-8c7e-5634788377be.png)是方差(![](assets/a72299da-6fbf-4b73-ac1c-ae7ceaea6f87.png)是标准差)。</sub>

接下来，我们将介绍一个使用高斯分布密度估计进行异常检测的一般方法示例:

1.  假设我们得到了一个新的例子， <sub>![](assets/2967b0b4-ba9a-44cc-bb47-3054ccfa3f31.png)。</sub>
2.  选择规则的特征![](assets/a71b33cd-5c54-4a6f-bb25-ba854c3405a9.png)，这意味着它们决定异常行为。
3.  拟合![](assets/ca69595c-b5f9-48b9-92ed-1d37812c5b3a.png)和![](assets/279683df-9526-45d1-bed5-281f693095e6.png)参数。
4.  计算 <sub>![](assets/56503cdd-5622-45c8-b951-ef8fa2080190.png)</sub> 用一个方程计算![](assets/a4d07791-8d80-431e-86f4-40d442cf2acd.png)在高斯分布中的概率。
5.  通过与阈值![](assets/291f13b4-2a44-4786-98d5-d920002100d7.png)进行比较，确定![](assets/b5d2d080-3ee5-4c4c-8294-62851a75556e.png)是否为异常；参见**异常模型**公式。

下图显示了正态分布数据的高斯分布密度估计示例:

![](assets/57bc4786-360e-437b-b8cb-1ec6ada5e973.png)

在这种方法中，我们假设所选特征是独立的，但是通常在真实数据中，它们之间存在一些相关性。在这种情况下，我们应该使用多变量高斯分布模型，而不是单变量模型。

以下公式给出了多元高斯分布中![](assets/a612b645-2808-40f0-9f52-00066c5c1e73.png)的概率:

![](assets/32a24505-f23e-4141-a459-75946b0375a6.png)

这里，![](assets/e066a7ea-fa54-4bbc-b37a-6bccecf4b69f.png)是均值，![](assets/bb30a437-f7a2-44f5-8e6b-fe2b4eb30d10.png)是相关矩阵，![](assets/4bd510b4-6457-4e57-aa9d-2c580e5d190c.png)是矩阵的行列式，![](assets/8f5508cf-4d89-449b-81e4-6299a9008d26.png):

![](assets/04b31ef7-031d-4a43-a2a3-5297d6083bd9.png)

下图显示了具有相关数据的数据集的单变量和多变量高斯分布估计模型之间的差异。请注意，分布边界如何用蓝色覆盖常规数据，而异常用较浅的颜色标记:

![](assets/fcc049b6-88af-432c-b63e-e2fae3913f40.png)
...
![](assets/793d2538-4329-40da-ad95-c597be0d8c22.png)
...
![](assets/580b91e1-fcd5-4413-a201-af327ee1dec4.png)
...
![](assets/207f86f1-c7c6-428e-91e8-1fc5a3377f51.png)
...
![](assets/352b2efc-31e6-4ab3-99d3-3f6b9562d467.png)
...
![](assets/9b93b254-31b0-46c2-adf7-b86737e8ce63.png)
