...
*   *N* :评审团成员人数
*   <sub>![](assets/99920d86-e1bb-494b-abc0-a40a8e03bb0e.png)</sub> :陪审团成员做出正确决定的概率
*   ***μ*** :整个陪审团做出正确决定的概率
*   **m** :陪审团成员的最低多数:

![](assets/b7e311fb-d924-47ce-85f8-05ba8eabb894.png)

*   <sub>![](assets/a530e995-8e68-4174-872f-90274739133c.png)</sub> :由 *I* 得到的 *N* 的组合数:

![](assets/e3a84c17-bdb6-4bdd-9f2a-c49a53a619ac.png)

若 <sub>![](assets/84b87bd7-659b-4e31-a506-3c5e2f6bce1b.png)</sub> 则 <sub>![](assets/e7876376-785e-41a3-a6e4-d4a567e9fa88.png)为</sub>

若 <sub>![](assets/9d656039-d75c-4fa3-8d72-166723e0c5df.png)</sub> 则 <sub>![](assets/dbc347d8-6cc8-4e9c-92c8-0bb20591b2ec.png)为</sub>
...
假设有一个大小为 *M* 的 *X* 数据集。均匀地从数据集 *N* 个对象中获取并返回每个对象。在选择下一个之前，我们可以生成 *N* 个子数据集。这个程序是指 *N* 次，我们选择任意一个样本对象(我们假设每个对象都是*以相同的概率 <sub>![](assets/242784e4-3602-4271-a8ca-c4f01abe668f.png)</sub> 拾取*，每次都是从所有原始的 *M* 个对象中选择。
...
假设有 *X* 训练数据集。借助自举，我们可以生成 <sub>![](assets/b6159b9b-9177-470e-be05-62103d9ea942.png)</sub> 子数据集。现在，在每个子数据集上，我们可以训练我们的 <sub>![](assets/d3ff8930-4f7b-4c13-98f7-af82ea58feb4.png)</sub> 分类器。最终的分类器对这些分类器响应进行平均(在分类的情况下，这对应于投票)，如下所示: <sub>![](assets/04d3ed1f-6a73-459e-b5f9-f194f98c529e.png)</sub> 。下图显示了该方案:

![](assets/7ee6b0b2-719d-42eb-95d5-c52abf630297.png)

用简单的算法 <sub>![](assets/ac7f7713-68ad-468b-9b30-1e807c2948de.png)</sub> 考虑回归问题。假设所有 *y(x)* 对象都有一个真答案函数，在 <sub>![](assets/66785179-0fad-4eca-ab32-bd1e2c8c1bfc.png)</sub> 对象上也有分布。在这种情况下，我们可以将每个回归函数的误差写成如下:

![](assets/52a633e9-fd37-45a0-a86c-b14deb814f3d.png)

我们也可以将**均方误差** ( **均方误差**)的期望值写成如下:

![](assets/6c7917c7-d986-4e2f-9a0c-195f34950528.png)

构建的回归函数的平均误差如下:

![](assets/b777c8ca-0046-4f58-bfcb-dad2f82c7c05.png)

现在，假设误差是无偏和不相关的，如下所示:

![](assets/3380cb65-c67a-4cea-8164-e927f91691c6.png)

现在，我们可以编写一个新的回归函数，对我们构建的函数的响应进行平均，如下所示:

![](assets/4a79c1d9-61cb-4fa3-a96f-da6a115c7ea9.png)

让我们找到它的**根 MSE** ( **RMSE** )来看看求平均值的效果，如下:

![](assets/d5308f56-1485-431d-99bd-1797cd75ff49.png)
...
假设我们有一组对，其中每对由属性 *x* 和目标变量 *y* 、<sub>T5</sub>组成。在这个集合上，我们恢复了 <sub>![](assets/ea48de4c-1554-4eac-b831-55a470e26537.png)</sub> 形式的依赖。我们通过近似 <sub>![](assets/e013e508-6d13-45e8-822e-430af7bff86f.png)</sub> 来恢复它。为了选择最佳近似解，我们使用特定的损失函数形式 <sub>![](assets/a539b4d2-278b-4699-b819-12540f2d4cc3.png)</sub> ，我们应该优化如下:

![](assets/540afbcd-d0aa-4497-83f9-00c3dce3e459.png)

由于可用于学习的数据量有限，我们也可以用数学期望改写表达式，如下所示:

![](assets/a348fc46-52d9-4f50-9a90-38d40b97bcd0.png)

我们的近似不准确。然而，boosting 背后的想法是，可以通过将另一个模型的结果添加到模型中来修正其误差，从而改进这种近似，如下所示:

![](assets/5cd85e0c-75b2-44bc-8b12-36c2767cb293.png)

以下等式显示了理想的误差校正模型:

![](assets/9f14eaf8-a0a2-434d-8154-52df77abd3ce.png)

我们可以把这个公式改写成下面的形式，这样更适合矫正模型:

![](assets/9642408b-29ce-4289-92fd-3fdd1b6cd752.png)

基于前面列出的假设，提升的目标是近似![](assets/1de1a33e-02ec-4a5a-bb1f-bfc0cce29da8.png)，使其结果尽可能接近*残差* ![](assets/a03d2a26-6a0e-4cbf-9465-1a2f6e78a41c.png)。这样的操作是按顺序执行的，也就是说，![](assets/ff2e71fb-aba9-4fab-895a-e55e8b657004.png)改进了
之前的![](assets/b4d29c62-3845-442e-a1c6-9a99442c7044.png)功能的结果。

这种方法的进一步推广允许我们将残差视为损失函数的负梯度，特别是形式为![](assets/7aabd92d-44b8-405e-9a1c-fe4ec7c76908.png)。换句话说，梯度提升是一种用损失函数及其梯度替换的 GD 方法。
...
![](assets/6d0b7c50-ab59-4ba9-a007-b27af5d78898.png)

这是![](assets/2d613568-632c-4a83-be54-c4b00eb3b046.png)类的![](assets/2b5ad6b9-e87c-487c-8689-821b69b81c41.png)函数之和；它们统称为**弱模型**(算法)。这样的近似是从初始近似开始顺序执行的，初始近似是某个常数，如下所示:

![](assets/2967405b-1049-4c50-a77e-0c52d983cdf9.png)

不幸的是，在任意损失函数的每一步选择![](assets/c22cd770-eabb-4f46-b423-80469fd4873d.png)最优函数是极其困难的，因此使用了一种更直接的方法。其思想是使用 GD 方法，通过使用可微![](assets/5b4f9d5b-a2fe-417d-9e86-596fc7a7c18a.png)函数和可微损失函数，如下图所示:

![](assets/874f8f6f-1fb3-470e-b517-76dc1b724359.png)

然后，升压算法形成如下:

1.  用常数值初始化模型，如下所示:

![](assets/917e3e06-7390-49e0-9080-9a0d2a63e4cf.png)

2.  重复指定的迭代次数，并执行以下操作:

*   计算伪残差，如下所示:

![](assets/9c55987d-e36a-4b00-8643-8879597058dd.png)

这里， *n* 为训练样本数， *m* 为迭代数， *L* 为损失函数。

*   用表格 <sub>![](assets/6d202286-2262-47bf-986e-63291f66b731.png)</sub> 的数据在伪残差上训练基本算法(回归模型) <sub>![](assets/062ac524-08d0-40fd-b716-24a7d7f9f170.png)</sub> 。
*   通过求解该形式的一维优化问题计算![](assets/30840d8f-d7da-46bd-a2b8-2e334fcbd3ae.png)系数，如下所示:

![](assets/9953552b-9482-440d-bbab-485235d2b648.png)

*   更新模型，如下所示:

![](assets/89dbb705-f29c-4f69-a40a-9718a759d901.png)

该算法的输入如下:

*   <sub>![](assets/32fdc21c-afbc-4301-8331-9fcef825601c.png)</sub> 数据集
*   *M* 次迭代的次数
*   *L( y，f )* 损失函数带有一个解析书写的梯度(这种形式的梯度允许我们减少数值计算的次数)
*   *h* *(x)* 基本算法的函数族的选择，以及它们的训练和超参数的过程

初始近似值的常数，以及![](assets/08b62dcf-219e-4a13-b401-ddfc1d2f1c3f.png)最佳系数，可以通过二分搜索法或相对于初始损失函数的另一种线搜索算法(而不是梯度)找到。

回归的损失函数示例如下:

*   ![](assets/169eb108-715e-4b39-bbdb-cf731721b0aa.png) : An *L <sub> 2 </sub>* 损失，也叫高斯损失。这个公式是经典的条件均值，也是最常见、最简单的选项。如果没有额外的信息或模型可持续性要求，则应使用它。
*   ![](assets/ecafc9ed-ea3d-400e-a30d-ee2149631589.png) : An *L <sub> 1 </sub>* 损失，也叫拉普拉斯损失。这个公式，乍一看，不是很可微，决定了条件中位数。正如我们所知，中位数更能抵御异常值。因此，在一些问题中，这个损失函数是优选的，因为它不像二次函数那样惩罚大的偏差。
*   ![](assets/29a745cd-e472-40c8-8865-d3826eecf637.png) : An *L <sub> q </sub>* 亏损，也叫分位数亏损。如果我们不想要条件中位数，但想要条件 75 %分位数，我们将使用![](assets/a247d352-9c48-4a7e-8355-b36279f38eb8.png)这个选项。这个函数是非对称的，并且会惩罚更多的观测值，这些观测值会出现在我们需要的分位数的一边。

分类的损失函数示例如下:

*   ![](assets/ef09553e-c5c9-4a7c-b229-01f88619d709.png):这是逻辑损耗，也叫**伯努利损耗**。这个损失函数的一个有趣的性质是，我们甚至惩罚正确预测的类标签。通过优化这个损失函数，即使所有的观测值都被正确预测，我们也可以继续对类进行距离分类并改进分类器。该函数是二进制分类任务中最标准和最常用的损失函数。
*   ![](assets/f214ec51-e46f-48f6-b9fa-2d376cfbb1f9.png):这是 AdaBoost 损失。碰巧的是，使用该损失函数的经典 AdaBoost 算法(在 AdaBoost 算法中也可以使用不同的损失函数)相当于梯度增强。从概念上讲，这种损失函数与逻辑损失非常相似，但它对分类错误具有更强的指数惩罚，并且使用频率较低。
...
2.  在一般情况下，在 *t* 节点定义一个![](assets/fdfdb36b-71d0-4b19-ac64-5e5a55798f5e.png)决策规则，该规则考虑一定范围的值。这个范围分为 *R <sub> t </sub>* 不相交的对象集合:![](assets/f292ded4-fa7d-4dc9-9932-02ed2637bc6a.png)，其中 *R <sub> t </sub>* 是节点的后代数量，每个![](assets/b50b9782-b5c9-4407-aa57-44809543c062.png)是落入![](assets/f171c4e4-e9ad-4ad0-997e-4735c5888288.png)后代的一组对象。
3.  根据选择的规则划分节点中的集合，并对每个节点递归地重复该算法。

大多数情况下，![](assets/d03f30a1-4202-467e-b039-6d9ff23e67ac.png)决策规则仅仅是特征，即![](assets/76007051-781a-44b6-9ce7-3c9e44990c95.png)。对于分区，我们可以使用以下规则:

*   ![](assets/871b98f9-92b7-41c1-b948-f99ee1826738.png)为选定的边界值 <sub>![](assets/856ff958-b6e0-4f3c-9393-01b13040c059.png)</sub>
*   ![](assets/3672a249-b74d-4e4a-bc2d-245895a9c5df.png)，其中![](assets/9f4a5d58-5310-427a-9438-d0e7e663f8d1.png)是向量的标量积。事实上，这是一个角值检查
*   ![](assets/1610e02b-de27-484d-8e4d-d820c987cdea.png)，其中
    距离![](assets/3a95b14e-64a0-422b-b43d-f8438e4c2ee4.png)在某个度量空间中定义(例如，![](assets/dec017d5-e209-4b53-965d-1af4941e5a9f.png))
*   ![](assets/e5ce3173-c0ce-4ace-b982-b0f3e034d197.png)，其中![](assets/191d14a8-3fc4-4a78-a547-de530b87a205.png)为谓语

一般来说，您可以使用任何决策规则，但是那些最容易解释的规则更好，因为它们更容易配置。接受比谓词更复杂的东西没有什么特别的意义，因为在谓词的帮助下，您可以在训练集上创建一个 100%正确的树。

通常，选择一组决策规则来构建树。为了找到每个特定节点的最优解，我们需要引入一个衡量最优性的标准。为此引入了![](assets/d7dfd541-839c-4f16-b01a-ba1cb6b37625.png)度量，用于度量对象如何分散(回归)，或者类如何在特定的![](assets/a5c97b58-8f4d-4ed1-b25e-82c1bf7740f8.png)节点中混合(分类)。这项措施被称为**杂质功能**。需要根据一组决策规则中的所有特征和参数找到最大值![](assets/c2db4e3a-d871-4d59-b250-76e8d5aac237.png)，以便选择决策规则。通过这种选择，我们可以为当前节点中的对象集生成最佳分区。

信息增益![](assets/68aeaa36-f9c2-4913-8319-9cd3c94e24ac.png)是我们对于所选拆分可以获得多少信息，计算如下:

![](assets/1a1b844b-9b45-47f4-8cf9-08aa172a5562.png)

在上式中，以下公式适用:

*   *R* 是当前节点被分解成的子节点数
*   *t* 为当前节点
*   <sub>![](assets/004754d9-b84d-43eb-b942-efe65868d8d8.png)</sub> 是用所选分区获得的后代节点
*   <sub>![](assets/126629f1-98cb-4ed5-9cc9-f7f56281afad.png)</sub> 是训练样本中落入子 *** i *** 的对象数量
*   <sub>![](assets/b8146a9e-e70a-4172-98f2-f2e0beeff1e4.png)</sub> 是被困在当前节点的对象数量
*   <sub>![](assets/d52ab2b4-c7de-41ff-a61d-9a60915c09a7.png)</sub> 是物体被困在*t<sub>I</sub>*T7<sup>th</sup>T10】的顶点

我们可以使用均方误差或平均绝对误差作为回归任务的杂质函数。对于分类任务，我们可以使用以下功能:

*   基尼标准![](assets/46a7eb89-9a4b-4f25-84f8-4a3e75d0a4c7.png)作为错误分类的概率，特别是如果我们用它们在给定节点出现的概率来预测类
*   熵![](assets/794e4e6e-1edd-4977-a0f2-f64baf8ce664.png)作为随机变量不确定性的度量
*   分类错误![](assets/1af01b11-027d-4a6e-b357-a10df6ea71dd.png)作为分类中最有力类的错误率

在前面描述的函数中，![](assets/a5c0f165-b881-4c8c-a289-381157a012ef.png)是在节点**中遇到类 *** i *** 的*先验*概率，即类 *** i *** 落入 *** t *** 的训练样本中的对象数除以 *** t *** 中的对象总数(**
...
1.  我们为训练集建立一棵树。
2.  然后，我们通过构造的树传递一个验证集，并考虑任何内部节点 *t* 及其左右子节点![](assets/b1a2d508-1867-44c3-8a46-dac9625fca13.png)、![](assets/f8803333-d100-49ca-8644-3ec997640bbd.png)。
3.  如果验证样本中没有一个对象达到了 *t* ，那么我们可以说这个节点(及其所有子树)是不重要的，并使 *t* 成为叶子(使用训练集将这个节点的谓词值设置为等于多数类的集合)。
4.  如果验证集中的对象已经达到 *t* ，那么我们必须考虑以下三个值:

*   *t* 子树的分类错误数
*   来自![](assets/85353bbb-2027-4108-b1ec-cc3efeaf7c4d.png)子树的分类错误数
*   来自![](assets/4eb4f1f8-5cc7-4ab1-b673-1621d2439f7b.png)子树的分类错误数

如果第一种情况的值为零，那么我们将节点 *t* 作为叶节点，并对该类进行相应的预测。否则，我们选择这些值中的最小值。根据哪一个是最小的，我们分别执行以下操作:

*   如果第一个是最小的，什么也不做
*   如果第二个最小，用节点![](assets/fcf18b7f-370d-4dfc-889c-adb7b50634aa.png)的子树替换节点 *t* 的树
*   如果第三个最小，用节点![](assets/53e5e1fd-92ea-4675-8d7b-b8cd005524e2.png)的子树替换节点 *t* 的树
...
当训练对象的数量为 *N* 且特征的数量为![](assets/1c22866d-c313-43a3-bb2b-92d33ba6072e.png)时，进行如下操作:

1.  选择![](assets/8c42b63c-2ef2-4ef9-b770-6e017d67b35d.png)作为系综中单棵树的数量。
2.  对于每个单独的![](assets/078dc0d5-5c5b-4269-be1a-756e25f8edbc.png)树，选择![](assets/44bb2d2e-72bc-4fdd-8b36-90b33ce25e9b.png)作为![](assets/2ece986f-1c78-4db3-969c-823b86de59c6.png)的特征数量。通常，所有树只使用一个值。
3.  对于每棵树，使用自举创建一个![](assets/17b15b95-4581-4bae-82ad-cb23fb91a586.png)训练子集。

现在，从![](assets/fae0a56e-9d55-4a9a-9b94-50ff686f711e.png)样本构建决策树，如下所示:

1.  从源中选择![](assets/1f8e7240-856e-4b35-b877-85a191435de3.png)个随机特征，那么训练集的最优划分将把搜索限制在这些特征上。
2.  根据给定的标准，我们选择最佳属性，并根据它在树中进行分割。
3.  直到每片叶子中剩下的物体不超过![](assets/c73ad081-303c-4ca7-adeb-02d8158f1bad.png)，或者直到我们到达树的某个高度，或者直到训练集耗尽，树才会被构建。

现在，要将集成模型应用于新对象，需要通过多数投票或通过组合*后验*概率来组合单个模型的结果。最终分类器的示例如下:

![](assets/070b4c9d-b8df-45ce-95f4-a0834e3aa99a.png)

考虑算法的以下基本参数及其属性:

*   **树数**:树越多质量越好，但训练时间和算法工作量也成比例增加。通常，随着树数量的增加，训练集的质量会提高(甚至可以达到 100%的精度)，但测试集的质量是渐近线(因此，您可以估计所需的最小树数量)。
*   **分裂选择的特征数量**:随着特征数量的增加，森林的建造时间也增加，树木变得比以前更加均匀。通常，在分类问题中，选择的属性数量等于回归问题的<sub>![](assets/baa1fe02-c623-4a52-b25a-5eb8f3a5c0b6.png)</sub><sub>![](assets/4d93a40a-23ca-483c-adcb-5d22b4c19375.png)</sub>。
...
![](assets/36e34659-c22a-493d-9084-5fcb9d954223.png)

在下图中，我们可以看到，由于我们将`CART tree`深度增加到 3，泛化能力显著增强:

![](assets/2111a49c-a9f2-441d-83a5-35d8532c9647.png)
...
![](assets/8e7f1873-97b6-4274-b084-4543fc30ef99.png)
