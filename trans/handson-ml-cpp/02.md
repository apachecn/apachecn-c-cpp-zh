...
![](assets/14e5cfa6-895d-470c-91d6-8bdd8c19962b.jpg)
...
考虑如何在 C++中处理 CSV 格式。用 C++解析 CSV 格式有很多不同的库。它们有不同的功能集和集成到应用程序中的不同方式。使用 C++库最简单的方法是使用只包含标题的库，因为这样就不需要构建和链接它们了。我们建议使用`Fast-CPP-CSV-Parser`库，因为它是一个小的单文件头文件库，所需功能最少，可以很容易地集成到开发代码库中。
...
*   另一种方法是连续解析文件，并为用户提供**应用程序接口** ( **应用编程接口**，以处理与文件解析过程相关的每个事件。第二种方法通常被称为“T4 简单应用编程接口”。尽管它有名字，但它也是一种用于非 XML 数据的通用方法。
...
**标准化**是使数据的均值为零，标准差等于 1 的过程。标准化向量的公式为![](assets/9f9888de-447e-4286-a02c-eee4da5e0b20.png)，其中![](assets/b932f86b-343e-44c9-8582-0abfa5919211.png)为原始向量，![](assets/414687a0-b5bb-4859-8d2d-0e0b0f2adf3a.png)为用公式![](assets/6c7c28c9-120c-4a11-a3cf-17b5508cdc9d.png)计算的![](assets/dacb38b5-734a-4730-95ea-e1027042fda4.png)的平均值，![](assets/f05795dd-44eb-400d-b100-48b3bc67ea18.png)为用公式![](assets/828ac954-4f61-4119-a133-c9f631dd815b.png)计算的![](assets/7032a37c-a43e-48c9-bab2-29902afbc061.png)的标准差。

**最小-最大归一化**或**重新缩放**是使数据符合`[0, 1]`范围的过程。我们可以使用以下公式进行重新缩放:

![](assets/0e8c3a21-0066-409f-b7e5-7a03fb120c4c.png)

**均值归一化**用于将数据拟合到范围`[-1, 1]`中，因此其均值为零。我们可以使用以下公式进行均值归一化:

![](assets/3c2fb868-48a9-44ab-aa10-6428d69de0fe.png)

考虑我们如何实现这些规范化技术，以及可以使用哪些机器学习框架函数来计算它们。

我们假设这个矩阵![](assets/70f70cf1-763e-4994-a106-8bb1345623a4.png)的每一行是一个训练样本，每一列的值是当前样本的一个特征的值。
...
请注意，`Dlib::mat()`函数对于不同来源的矩阵创建有不同的重载。此外，如果 *m* 是输入矩阵，我们使用`reciprocal()`函数生成![](assets/d79c7944-d7c3-4129-8901-04c77c28ce11.png)矩阵。
