...
![](assets/6a12fff9-005f-46e1-940e-195a5daf1277.png)

# 平方欧几里德距离

平方欧几里德距离具有与欧几里德距离相同的属性，但是它也给远处的值增加了比较近的值更多的重要性(权重)。以下是平方欧几里得距离的公式:

![](assets/fa784879-d232-4f1d-b30f-b1f2fe9d310d.png)

# 曼哈顿距离

曼哈顿距离是坐标的平均差值。在大多数情况下，它的值给出了与欧氏距离相同的聚类结果。然而，它降低了远处值(异常值)的重要性(权重)。下面是曼哈顿距离的公式:

![](assets/85c8824c-bf7f-4948-a695-f0b9d362a88f.png)

# 切比雪夫距离

当我们需要将两个物体分类为不同时，当它们仅相差一个坐标时，切比雪夫距离会很有用。以下是切比雪夫距离的公式:

![](assets/5c8d2f13-1701-4019-9476-3fc417c802c0.png)

下图显示了不同距离之间的差异:

![](assets/fd828ec2-9de2-4618-a260-26539a7044d2.jpg)
...
![](assets/bbac96af-193c-4859-820f-a37dd9054973.png)

通过去掉 *C* 和 *D* 之间的链接，长度为 6 个单位(距离最大的边)，我们得到两个集群:{ *A* 、 *B* 、 *C* }和{ *D* 、 *E* 、 *F* 、 *G* 、 *H* 、 *I【我们可以通过移除长度为 4 个单位的边缘 *EF* 将第二个簇再分成两个簇。*

多层聚类算法是基于在对象(顶点)之间的某个距离级别上识别图形的连通分量。阈值 *C* 定义距离等级——例如，如果物体之间的距离是 <sub>![](assets/202956f4-0da8-45ba-b8e0-979294ae1851.png)</sub> ，那么![](assets/28384884-5d08-43f2-87ca-7c1ab9cb8752.png)。

图层聚类算法生成图形 *G* 的一系列子图形，这些子图形反映了聚类 <sub>![](assets/de9391b2-5bac-4168-b248-3d803e83a75c.png)</sub> 之间的层次关系，适用于以下情况:

*   <sub>![](assets/f2b2974c-03f0-41ad-940a-9cea6ff95afa.png)</sub> :一个![](assets/ef0880e2-6fb9-4c19-b906-805e8c552bc4.png)级的子图
*   <sub>![](assets/e1ec1c4a-23ba-466b-ac20-0e8f57c14884.png)</sub>
*   <sub xmlns:epub="http://www.idpf.org/2007/ops">![](assets/842ea6e2-bdd8-473b-a727-0afc78026ed5.png)</sub> :距离的*t*T5 门槛
*   ![](assets/79ec75c3-ff01-406a-9527-3ec14da084eb.png):层级的数量
*   <sub>![](assets/d0ae07c8-6308-4f9c-a6f8-d75b1192a974.png)</sub> ，o:一组空的图边，当 <sub>![](assets/9205833b-5d09-4653-82c7-03b93ea7688f.png)</sub>
*   ![](assets/3575f0ad-d578-4dff-a517-8f81bd3a145b.png):距离无阈值的物体图， <sub>![](assets/2b295eb0-f272-4c72-a1f7-6ae86cac0f7c.png)</sub>

通过改变 <sub>![](assets/1864d2bd-03a5-46b3-82c7-7ffdd3439ba5.png)</sub> 距离阈值，其中 <sub>![](assets/2a3e95f0-503a-4752-9acb-1199dc77ad57.png)</sub> ，可以控制生成的聚类的层次深度。因此，多层聚类算法可以创建平面和分层数据分区。
...
![](assets/02a8b315-fe12-45d9-8723-86ded7abe812.png)
...
*   一个物体的 <sub>![](assets/fce26991-e6dd-436f-9d91-307c3b75800f.png)</sub> 是一个物体的 <sub>![](assets/eb0d1246-dcad-42a2-a68c-f0f9e73e177d.png)</sub> 半径的邻域。
*   根对象是其 <sub>![](assets/06c8efdb-9bf2-4e4f-bc7d-78cfa0debf24.png)</sub> 包含最小非零数量对象的对象。假设这个最小值等于一个名为 *MinPts* 的预定义值。
*   如果 *p* 在 *q* 的<sub xmlns:epub="http://www.idpf.org/2007/ops">![](assets/1522c0f0-43b9-42ce-8dee-795c764f55e2.png)T8 中，并且 *q* 是根对象，则 *p* 对象可以从 *q* 对象直接密集访问。</sub>
*   如果存在一系列 <sub>![](assets/a38d18f6-c83d-4009-9dc4-567c6425b9b0.png)</sub> 对象，其中 <sub>![](assets/73d5f6c4-56e6-492c-b936-8873cc63366d.png)</sub> 和 <sub>![](assets/83d62fab-d367-4d6c-8437-b32149011268.png)</sub> ，则可以从给定的![](assets/f7fd6982-5cfb-4bea-9e40-d2362340f9e2.png)和 *MinPts* 对象密集访问 *p* 对象，从而可以从<sub>![](assets/e142006c-02cd-48ab-8a7b-2b87815cd31c.png)</sub><sub>![](assets/4220448a-7f13-41eb-8da2-141fbaf1c154.png)</sub>直接密集访问 <sub>![](assets/82f3d230-4a98-4731-9f6c-341ef391cff9.png)</sub> 。
*   对于给定的![](assets/fd191bdf-9e8d-4344-8173-d95825c901f6.png)和 *MinPts* 来说，如果存在 *o* 对象，则 *p* 和 *q* 可以从 *o* 密接，则 *p* 对象与 *q* 对象密接。

DBSCAN 算法检查每个对象的邻域以搜索聚类。如果 *p* 对象的 <sub>![](assets/0cf63a6a-1dad-4fbe-804a-e8bf32568552.png)</sub> 包含比 *MinPts* 更多的点，则使用 *p* 对象作为根对象创建一个新簇。然后，DBSCAN 迭代地从根对象中收集可直接密集访问的对象，这可以导致几个可密集访问的集群的联合。当不能向任何集群添加新对象时，该过程结束。

与基于分区的方法不同，DBSCAN 不要求预先指定集群的数量；它只需要![](assets/7706d377-6e70-498e-836c-9cb109bef1d1.png)和 *MinPts* 参数的值，直接影响聚类的结果。这些参数的最优值很难确定，尤其是对于多维数据空间。此外，这种空间中的数据分布通常是不对称的，这使得不可能使用全局密度参数进行聚类。对于多维数据空间的聚类，有基于 DBSCAN 算法的**subclou**(**子空间聚类**)算法。
...
![](assets/08def3f8-df70-49af-8a4f-0ba71e57b89c.png)
...
![](assets/a1227e48-ee3f-43dc-b0e7-2cbca0fa2efa.png)
...
![](assets/996995e0-1033-4a08-a4fd-1d3d7d855f1e.png)
...
![](assets/cfa308f1-c4d7-4827-9328-1140432754ca.png)
...
![](assets/02fe364d-3535-418e-990a-0da16148fc02.png)
...
![](assets/a64f3656-0864-4484-91aa-702713382b39.png)
...
![](assets/c80f0671-584f-4698-854f-09e109e4669a.png)
...
![](assets/7d46014c-9e2c-467b-b2c5-6b74e38851e8.png)
...
![](assets/5e6f82d6-db68-4680-acb3-6d18e1c84296.png)
