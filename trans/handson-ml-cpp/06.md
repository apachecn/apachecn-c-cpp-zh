...
为了将我们的数据从![](assets/9af25c98-ff94-4af5-8e8d-e3bffeb5f641.png)降维到![](assets/56b06bb8-039e-40bf-9077-1d2123b3caaa.png)，我们需要选择这样一个椭球体的顶部![](assets/c1b67987-fa80-40fc-ac01-684d497e227e.png)轴，按照沿着轴的离散度降序排列。首先，我们计算原始特征的方差和协方差。这是通过使用协方差矩阵来完成的。根据协方差的定义，对于两个符号![](assets/0ec1e741-a68d-4398-8f87-1e85193bce4c.png)和![](assets/6a227d9e-55ff-43f5-b845-0ad2f1f5fb74.png)，它们的协方差应该如下:

![](assets/b1657a92-634a-4abc-97be-8a475ee2edc4.png)

这里，![](assets/72dbd737-5dd9-4870-8f64-35588796c900.png)是![](assets/24c794f9-777f-4736-a1e0-4b2a27f22e26.png)特征的平均值。

在这种情况下，我们注意到协方差是对称的，向量本身的协方差等于它的离差。因此，协方差矩阵是对称矩阵，其中对应特征的离散位于对角线上，对应特征对的协方差位于对角线之外。在矩阵视图中![](assets/ebb7578f-b547-4fcb-9846-f5e6f360bab4.png)是观测矩阵，我们的协方差矩阵如下:

![](assets/e5e0fb61-8116-4717-8945-2bcb06dd9582.png)

协方差矩阵是多维随机变量情况下方差的推广，它也描述了随机变量的形状(分布)，方差也是如此。像线性算子这样的矩阵有特征值和特征向量。它们之所以有趣，是因为当我们作用于相应的线性空间或用我们的矩阵对其进行变换时，特征向量保持不变，它们只与相应的特征值相乘。这意味着它们定义了一个保持不变的子空间，或者当我们对其应用线性算子矩阵时*进入自身*。形式上，一个特征向量，<sub>![](assets/70fc5ecd-031e-44d5-bf52-5d9810a6f233.png)</sub>与一个矩阵的特征值被简单地定义为 <sub>![](assets/ba9bcd06-cc1e-4820-b7c3-754bad6299f1.png)</sub> 。

我们样本的协方差矩阵， <sub>![](assets/3d50eba9-7e9d-4b38-ad70-18d58feb3a6a.png)，</sub>可以表示为一个乘积， <sub>![](assets/297ba120-1a64-4e09-91d3-e544ee3a43d9.png)</sub> 。根据瑞利关系，我们的数据集的最大变化可以沿着该矩阵的特征向量实现，该特征向量对应于最大特征值。对于更多维度上的投影也是如此–投影到 *m* 维空间上的方差(协方差矩阵)在具有最大特征值的 <sub>![](assets/6bbd6e69-52bb-48a7-8fe5-56b3700a96c4.png)</sub> 特征向量的方向上最大。因此，我们想要投影数据的主成分仅仅是这个矩阵的特征值的相应顶部 *k* 片段的特征向量。

最大的向量有一个类似于回归线的方向，通过将我们的样本投影到它上面，我们会丢失信息，类似于回归的剩余成员的总和。有必要进行操作， <sub xmlns:epub="http://www.idpf.org/2007/ops">![](assets/1a3a14db-ca73-42b8-a198-985ed3d39927.png)</sub> (向量长度(量级)应等于 1)，执行投影。如果我们没有一个单独的向量，而是有一个超平面，那么我们取基向量的矩阵，而不是向量，<sub xmlns:epub="http://www.idpf.org/2007/ops">![](assets/638e98dd-b93b-486f-bc38-4b1979fc8d15.png)</sub>![](assets/73a37eb5-37ae-4191-9f23-82e0427a5d4f.png)。得到的向量(或矩阵)是我们观察到的一系列投影；也就是说，我们需要将我们的数据矩阵乘以基向量矩阵，我们得到数据的正交投影。现在，如果我们将数据矩阵的转置和主成分向量的矩阵相乘，我们将在空间中恢复原始样本，在空间中我们将它投影到主成分的基础上。如果组件的数量小于原始空间的维度，我们会丢失一些信息。
...
![](assets/10414122-4244-4309-8831-e099ef7ccfc5.png)

这里，![](assets/dae4918b-8615-41bf-a06b-d9abda5a0602.png)是一个序为![](assets/cc4427c8-a042-4cae-a9e8-53aef5ca6e5e.png)的酉矩阵，![](assets/2abdaf7e-52a8-447e-8715-db33fd6fe4cb.png)是一个主对角线上的大小为![](assets/9eb39eb5-cab4-40d9-b002-8929e012278e.png)的矩阵，这里有称为奇异值的非负数(主对角线之外的元素为零——这种矩阵有时称为矩形对角矩阵)。![](assets/dfac78d8-476a-45fa-8ca8-d993db485bc9.png)是一个埃尔米特共轭的![](assets/b2f778f7-f93b-4941-9464-5dff8f62b289.png)阶矩阵![](assets/c7bea52d-06ab-43d0-8496-414d30e5b9ce.png)。矩阵![](assets/a0cea3c7-743e-4fdb-9ad8-acaee088f488.png)的![](assets/23df664e-b7ee-4b12-9c80-bbd67ba68231.png)列和矩阵![](assets/9c7a597d-bb0e-4b48-8bc7-d2eeb50ab350.png)的![](assets/aa253c1e-d630-41db-9f24-c7b2154ea525.png)列分别称为矩阵![](assets/cad991ff-fb51-4e43-b52e-0652f3eff5fb.png)的左右奇异向量。为了减少维数，矩阵![](assets/b09f41e6-54b6-4dfb-9b55-6108ec4709f7.png)很重要，当矩阵的元素上升到二次幂时，可以解释为每个分量放入联合分布的方差，它们按降序排列:![](assets/ba5201da-fe39-4056-938c-97bfcd9ef3db.png)。因此，当我们选择奇异值分解中的分量数时(如主成分分析)，我们应该考虑它们的方差之和。

奇异值分解和主成分分析之间的关系可以用以下方式描述:![](assets/0e089bb4-d092-4464-839c-6531b1313f94.png)是![](assets/ca15739e-addc-4231-baf9-175cf6cba61d.png)给出的协方差矩阵。它是一个对称矩阵，所以可以对角化:![](assets/ea4e918d-73c8-4428-b236-f225433b9afa.png)，其中![](assets/11ae5946-76b7-4c40-ade0-73b47e4064ec.png)是特征向量的矩阵(每列是一个特征向量)![](assets/39e19da1-f1bb-4b16-844a-cdea9735438b.png)是特征值的对角矩阵，![](assets/0de6aebf-b4c6-45cf-b43b-d8595a628370.png)，在对角线上按降序排列。特征向量被称为数据的主轴或主方向。数据在主轴上的投影称为**主成分**，也称为**主成分得分**。它们是新转换的变量。![](assets/f273e853-9205-4e87-a3f6-762ab3dcfeb5.png)主成分由![](assets/b0bc04d8-8bd3-4f04-b735-798b17b3c066.png)的![](assets/aaa8a702-32e1-447e-83d4-fc0eda90d2a7.png)列给出。新主成分空间中![](assets/ff80b5f1-8fed-4109-92e1-0cad40b2855f.png)数据点的坐标由![](assets/df7afb8c-8926-4aa5-8650-583fc28a6d2a.png)的![](assets/d416aefc-c90d-4080-870c-c451e124ad36.png)行给出。

通过对<sub>![](assets/7ebbb647-e91f-4938-a451-21bcd97dcac2.png)</sub>进行奇异值分解，我们得到<sub>![](assets/2cd46de2-5c2f-49ef-89a9-efb85e92eb1b.png)</sub>，其中![](assets/6e8f9736-364e-4654-9e04-5b0030d3f94d.png)是酉矩阵，![](assets/0842e822-4126-4cca-ac4f-db793c2215e1.png)是奇异值的对角矩阵，![](assets/481a465e-9ee0-42fc-afea-2625ba5bda3c.png)。我们可以观察到<sub>![](assets/cdbbd34e-3604-4c15-a08b-29ff7612a320.png)</sub>，这意味着右奇异向量![](assets/91e4a441-b41e-48ce-9e3b-08618b515372.png)是主方向，奇异值通过 <sub>![](assets/f47584cd-78c8-48c4-8e6e-228c1222f73b.png)</sub> 与协方差矩阵的特征值相关。主要成分由![](assets/67636c0d-e8b1-499a-a747-ea54a34cae0c.png)给出。

# 独立成分分析

提出了**独立分量分析** ( **ICA** )方法作为解决**盲信号分离** ( **BSS** )问题的方法；即从混合数据中选择独立信号。我们来看一个 BSS 任务的例子。假设我们有两个人在同一个房间里说话，产生声波。我们在房间的不同地方有两个麦克风，记录声音。分析系统从两个麦克风接收两个信号，每个信号都是两个声波的数字化混合——一个来自说话的人，另一个来自其他噪音(例如，播放音乐)。我们的目标是从进入的混合物中选择我们的初始信号。数学上，这个问题可以描述如下。我们以线性组合的形式表示输入混合物，其中![](assets/3552ea62-9609-4731-a38c-018ed3d6a107.png)表示位移系数，![](assets/e4c80c21-f9e2-4f49-80bc-5f826e422be0.png)表示独立分量的矢量值:

![](assets/302eaf1e-c2d3-4209-85df-e97e955e808b.png)

在矩阵形式中，这可以表示如下:

![](assets/45cdee39-deba-46c4-b2e1-0b40d25d3219.png)

在这里，我们必须找到以下内容:

![](assets/58edc396-a823-48b9-933b-3c6b1c7a0614.png)

在这个方程中，![](assets/622f65c5-93f7-4fdd-b478-d31de369f1f2.png)是输入信号值的矩阵，![](assets/ed68dd3c-db09-4248-b8ee-c046ecd8545d.png)是位移系数或混合矩阵，![](assets/2492387b-91f0-47fe-a8cc-43d8da6e8e11.png)是独立分量的矩阵。因此，问题分为两部分。第一部分是得到原始独立成分的变量![](assets/b62d6dfe-60d5-4fc0-a8fb-5c9370e98c54.png)的估计值![](assets/ebb76285-6d30-45e9-9232-ee2ec66769fe.png)。第二部分是找矩阵，![](assets/8ea4b855-9f1b-4ec0-9384-6d3f001c9c33.png)。这种方法的工作原理基于两个原则:

*   独立分量必须是统计独立的(![](assets/9d0c88d5-d48a-4cfd-99b2-ffc984f7a661.png)矩阵值)。粗略地说，一个独立分量的一个向量的值不影响另一个分量的值。
*   独立分量必须具有非高斯分布。

独立分量分析的理论基础是中心极限定理，该定理指出![](assets/114d98f9-8dca-438c-aa21-ff70819f1430.png)个独立随机变量之和(平均值或线性组合)的分布对于![](assets/47e0267f-e7d4-435e-b492-8f690be46463.png)接近高斯。特别地，如果![](assets/3773653b-1330-4147-a08c-e8fb59e273cf.png)是相互独立的随机变量，取自平均值为![](assets/67c0fab7-c8d1-4e17-90bc-7b829f99ec48.png)，方差为![](assets/a9ac2b8f-4856-4b3b-adbe-7bcca9c5fb6c.png)的任意分布，那么如果我们将这些变量的平均值表示为![](assets/b4cc30b1-e7fe-4d18-b95b-2b7dc63c44e1.png)，我们可以说![](assets/6f686c9b-156f-4d84-99d0-da12f9371194.png)接近均值为 0，方差为 1 的高斯分布。解决 BSS 问题，需要找到矩阵，![](assets/ed9922b8-4437-4a1b-b496-2bf25d9eb757.png)，这样![](assets/cbb877f0-4881-4494-a86e-584b065a73bd.png)。在此，![](assets/687e752d-7bf1-4d7b-83ba-51db00f4f0fb.png)应尽可能接近原始独立来源。我们可以把这种方法看作中心极限定理的逆过程。所有独立分量分析方法都基于相同的基本方法——找到一个最大化非高斯性的矩阵 **W** ，从而最小化 <sub>![](assets/f05dd3d7-147f-4eab-90cd-760887925fb5.png)</sub> 的独立性。

快速独立分量分析算法旨在最大化功能 <sub>![](assets/52f63c65-87db-4079-8d39-5d50e04a70f5.png)</sub> ，其中![](assets/5764ca43-2c00-40b5-b00a-bb632c702017.png)是![](assets/7de97e6c-11d1-4b1c-870d-1ca995bd1cf4.png)的组成部分。因此，我们可以将函数的方程改写为以下形式:

![](assets/66e50274-3d9d-4a39-8d84-4cf88eba8b13.png)

在这里， <sub xmlns:epub="http://www.idpf.org/2007/ops">![](assets/4a84fc96-4e05-419c-a355-973b56f259df.png)</sub> 矢是*I*T5】th 行的矩阵， **W** 。

独立分量分析算法执行以下步骤:

1.  选择*的初始值**w .**T3】*
2.  计算![](assets/2423157b-74d7-4478-87f2-a537f33bac8b.png)，其中![](assets/a8b33531-fa52-469c-a3eb-c7b39507fbc3.png)是函数的导数， *** G(z) *** 。
3.  正常化![](assets/a5dc51ff-781b-47b5-9afd-b9f270a4e738.png)。
4.  重复前两步，直到 ***w*** 停止变化。

为了测量非高斯性，快速独立分量分析依赖于非二次非线性函数***【G(z)***，它可以采取以下形式:

![](assets/120ac1d6-2360-4ce7-a1c0-c226b826628f.png)

# 线性判别分析

**线性判别分析(LDA)** 是一种多元分析，它允许我们同时估计两组或多组对象之间的差异。判别分析的基础是假设每个 *k* <sup>th</sup> 类的对象描述是多维随机变量的实例，该多维随机变量根据正态(高斯)定律![](assets/18c587e6-1ad7-44a9-8d24-5dcffad9602e.png)分布，具有平均值、 <sub>![](assets/f90f2a51-65d8-4fa3-8b09-b2eeeac9908b.png)、</sub>和以下协方差矩阵:

![](assets/c731e53d-01d6-4aa4-b8dd-17a83a3def69.png)

索引![](assets/b2153c95-79ad-4246-ad04-6769fc81313f.png)表示特征空间的维度。考虑两类情况下线性判别分析算法的简化几何解释。让判别变量![](assets/9da15086-f621-4250-9594-ed86a061619e.png)成为![](assets/7f788158-4895-4b1d-ba6a-62f5699a0cb9.png)维欧氏空间的轴。每个对象(样本)都是这个空间的一个点，坐标代表每个变量的固定值。如果两个类在可观察变量(特征)上彼此不同，它们可以被表示为所考虑的空间的不同区域中可能部分重叠的点的集群。为了确定每个类的位置，我们可以计算它的质心，它是一个虚点，其坐标是类中变量(特征)的平均值。判别分析的任务是创建一个额外的![](assets/52b121ef-906a-4747-8730-721cc03f1ba1.png)轴，该轴通过点云，使得其上的投影提供了两个类别的最佳可分性(换句话说，它最大化了类别之间的距离)。它的位置由线性判别函数(线性判别，LD)和权重![](assets/1238739a-2671-4673-8f57-368588a535ef.png)给出，确定每个初始变量的贡献![](assets/a642c5bd-50f4-40e8-a65a-c8d0917638cf.png):

![](assets/9f5103f5-a59c-40c6-bff1-e48084dea8ca.png)

如果我们假设类 1 和类 2 的对象的协方差矩阵相等，即![](assets/bfe8a121-11c7-4231-8319-a94275badd5c.png)，那么线性判别式![](assets/ee281083-1916-4442-a7f9-53f208ee0c77.png)的系数向量![](assets/8aaeacce-c9a8-4cb6-b568-b00add9777df.png)可以使用公式![](assets/3c811a08-87b9-413c-a6a2-83073a0deec2.png)来计算，其中![](assets/b5291ac1-da49-40ea-8d8a-a581237ad4b0.png)是协方差矩阵的逆，![](assets/733003ea-0671-4532-8de7-49e90fc8cb03.png)是![](assets/90d80bc0-9ee2-4e77-851a-32deddbad50b.png)类的平均值。得到的轴与穿过两组类对象质心的直线方程一致。广义马氏距离等于它们在多维特征空间中的距离，估计为![](assets/9f1ee950-6265-41d5-99d6-a8b68c5b1061.png)。因此，除了关于类数据的正态(高斯)分布的假设(这在实践中很少发生)之外，LDA 对于组内离差和相关矩阵的统计相等性有更强的假设。如果它们之间没有显著差异，则它们被合并成一个计算的协方差矩阵，如下所示:

![](assets/3ca501bf-8ece-4209-8bcd-a023608f811d.png)

这个原则可以推广到更多的类别。最终的算法可能如下所示:

![](assets/9768eee7-3fe6-4c4f-9dac-a3b42f6ad373.png)

类间散射矩阵是这样计算的:

![](assets/f2436529-496d-4211-b79c-a514da3b3ab8.png)

这里， <sub>![](assets/c69d9119-3427-4290-b190-ad07cabb1604.png)是</sub>所有物体(样本)的均值，![](assets/dda3a3e1-7829-4e38-ad52-5541128a71ef.png)是类的个数，![](assets/b062e35f-3fde-4bd0-92e9-f0474e218279.png)是 *i* <sup>th</sup> 类中的物体个数，![](assets/677e7a07-a012-43ef-a63c-b3fc76bc352c.png)是类内均值， <sub>![](assets/a8baba40-91eb-427f-ba40-45674105f687.png)</sub> 是 *i* <sup>th</sup> 类的散射矩阵，![](assets/c0c13a97-6e7e-4194-9633-b2be4d386aba.png)是对中矩阵，其中![](assets/2dc5d082-2270-4555-ae67-28a14d842469.png)是 *n* x

基于这些矩阵，计算![](assets/4257a5b4-b407-472c-b25b-fd53399eaaa0.png)矩阵，为其确定特征值和相应的特征向量。在矩阵的对角元素中，我们必须选择最大特征值的 *** s *** 并变换矩阵，只留下相应的 *** s *** 行。生成的矩阵可用于将所有对象转换到低维空间。
...
![](assets/aad2b943-8378-4140-a129-3e4ca8bc35ac.png)

这里，![](assets/9647ef76-b21e-4122-813d-86e5924a245f.png)是负载，![](assets/1a1ed60f-bfdd-4e4d-8453-ba68ac2b97e3.png)是因子。
...
**当除了相关矩阵之外，任意类型的对象相似性矩阵可以用作输入数据时，多维缩放(MDS)** 可以被认为是因子分析的替代。与其说 MDS 是一个正式的数学过程，不如说是一种有效放置对象的方法，从而在新的特征空间中保持对象之间的适当距离。MDS 新空间的尺寸总是大大小于原来的空间。MDS 用于分析的数据通常是从对象的成对比较矩阵中获得的。主要的 MDS 算法的目标是恢复所分析的特征空间的未知维度，![](assets/f65e0ffb-229c-4ecd-998a-26317424b9dd.png)，并为每个对象分配坐标，使得所计算的对象之间的成对欧几里德距离尽可能与指定的成对比较矩阵一致。我们说的是用正交变换的精度恢复新的约简特征空间的坐标，确保对象之间的成对距离不变。

因此，多维缩放方法的目的也可以公式化，以便显示由成对比较矩阵给出的原始多维数据的配置信息。这是作为较低维度的相应空间中的点的配置来提供的。

经典 MDS 假设未知坐标矩阵![](assets/a9953316-3709-4bb3-b1af-57df811a07ed.png)可以用特征值分解![](assets/0dc50bf4-1231-4b19-811f-591404e3c228.png)来表示。![](assets/2a8400ae-ed8d-4ddd-8a68-098ab0e66dce.png)可以通过使用双中心从邻近矩阵![](assets/8f3df3a1-5048-44fe-ab6a-e63aeea22040.png)(样本间距离的矩阵)中计算出来。通用 MDS 算法遵循以下步骤:

1.  计算邻近矩阵的平方，![](assets/fcd13b9c-2fd5-4a7c-891c-6aa3736a9274.png) <sub>。</sub>
2.  应用双居中， <sub>![](assets/87c9c894-9f5a-44cb-b051-81dfc1de17f9.png)，</sub>使用居中矩阵， <sub>![](assets/fc60c215-24bf-4a6a-a703-80a3169d1eff.png)</sub> ，其中![](assets/49afc51f-ebb3-4e92-b0dd-4deee80a7fe3.png)是对象的数量。
3.  确定![](assets/ae00e407-2815-4412-af20-f027d8d87749.png)的![](assets/285c9c49-c148-464e-b58d-16c3172bed0b.png)最大特征值![](assets/335be5be-cb7a-4b7c-9ce1-4860351fd93d.png)和相应的特征向量![](assets/b723c781-158e-4b4b-846e-a81d56da6fc5.png)(其中![](assets/b5994d0f-661d-41ba-b023-45c66f4be354.png)是输出所需的维数)。
4.  计算![](assets/0337db37-4ba9-4373-8d68-d1e8976d9952.png)，其中![](assets/e4c734cf-c4d5-4aa8-9bea-2e824c680f83.png)是![](assets/4933d983-3a01-4273-b5ee-159cc313c497.png)特征向量的矩阵，![](assets/47191df0-6c69-4062-8d1a-903460fd200d.png)是![](assets/d398a19f-2959-41cb-ab0b-54f3a7601ed5.png)的![](assets/23d0d426-22ae-4958-a418-eb1832ad65ca.png)特征值的对角矩阵。
...
经典的主成分分析是一种线性投影方法，如果数据是线性可分的，这种方法效果很好。然而，在线性不可分离数据的情况下，需要非线性方法。处理线性不可分数据的基本思想是将它投影到具有更多维度的空间中，在那里它变成线性可分的。我们可以选择一个非线性映射函数![](assets/1bdc8813-9953-47f2-abec-29bb06a3cf1f.png)，这样样本映射 *x* ，就可以写成![](assets/19e42739-920f-465f-9046-3cbe3cb4b28f.png)。这就是所谓的**内核函数**。术语**内核**描述了计算映射(在高阶空间中)样本 *x* 与![](assets/969a48d0-a284-4c52-8ca4-cd16194edf6f.png)的标量积的函数。这个标量积可以解释为在新空间中测量的距离。换句话说，![](assets/69b9671f-f6d9-40ea-a568-77e5d45eaf30.png)功能通过创建原始对象的非线性组合，将原始的 *d* 维元素映射到更高维的 *k* 维特征空间中。例如，在三维空间中显示 2D 样本![](assets/fa8b1ac1-0959-4dde-9504-dd8d99c83f27.png)的功能可能看起来像![](assets/94ded960-d743-4994-aad4-593be34ec105.png)。
...
1.  计算*核矩阵*等于![](assets/96cb6566-d352-480a-a425-613891bb0da1.png)。
2.  使其具有零平均值![](assets/8f487f18-a8a8-49a0-9575-87f6e80664d4.png)，其中![](assets/b804490b-c56d-44ed-b854-704da5ef4fd7.png)是 N×N 大小的矩阵，具有 1/N 个元素。
3.  计算![](assets/701ece1c-87e7-4062-a2f8-6558f4dba108.png)的特征值和特征向量。
4.  根据特征值，按降序排列特征向量。
5.  取最大特征值对应的![](assets/41460b50-4ac6-4270-97f2-ed6cbc574ea5.png)特征向量，其中![](assets/1197cc90-8333-4aae-84b2-331797242f5f.png)为新特征空间的维数。

这些特征向量是我们的数据在相应的主要组成部分上的投影。这个过程的主要困难是选择正确的内核并配置其超参数。两个常用的核是多项式核![](assets/5477abe5-e555-4431-83c6-285ac4ef9b59.png)和高斯核 <sub>【T2 核</sub>。
...
*   用于搜索大地测量距离的邻居数量![](assets/f3341592-8147-42c9-b078-6bf4209e822b.png)
*   最终空间的维度，![](assets/b25e9660-d83d-44a0-9d2b-684d26b83f92.png)

简而言之，IsoMap 算法遵循以下步骤:

1.  首先，它构建了一个表示测地线距离的图。对于每个点，我们搜索![](assets/dc1d768e-182b-43e2-b490-fbefd28154fe.png)最近的邻居，并从到这些最近的邻居的距离构建一个加权的无向图。边缘权重是到邻居的欧几里得距离。

2.  使用一种算法来寻找图中的最短距离，例如，Dijkstra 的算法，我们需要找到每对顶点之间的最短距离。我们可以把这个距离看作流形上的测地线距离。
3.  基于上一步获得的成对测地距离矩阵，训练 MDS 算法。
4.  MDS 算法将![](assets/0e5a7c55-b76f-407d-8380-746c16cf7706.png)维空间中的一组点与初始的一组距离相关联。

# Sammon 映射

**Sammon 映射**是最早的非线性降维算法之一。与传统的降维方法(如 PCA)相比，Sammon 映射不直接定义数据转换函数。相反，它只决定了转换结果(较小维度的特定数据集)与原始数据集结构的对应程度。换句话说，它并不试图寻找原始数据的最优变换；相反，它会搜索另一个更低维度的数据集，其结构尽可能接近原始数据集。该算法可以描述如下。假设我们有![](assets/b0f82c05-d6fe-41c4-884a-f02f705905f5.png)-维向量，![](assets/12f57fdb-ff67-4ccb-9a86-a6ae7579ce38.png)。这里，![](assets/8fc4bbb1-b4a6-43e4-a544-85a526e9b481.png)向量在![](assets/229300f5-c0b2-439a-86aa-5a7fe87fc693.png)维空间![](assets/9591ba00-3a5a-4fce-9741-3335f6ae4f3d.png)中定义，用![](assets/ed036570-33c8-48bf-a316-c218e4c9ddcf.png)表示。![](assets/924106e0-3c37-44d1-8166-1d9197affdf6.png)维空间中的向量之间的距离将由![](assets/2248a3f5-6a99-44aa-9ac6-1ba43f76cbca.png)表示，而在![](assets/ce20bd48-8540-4679-a981-a121156f149b.png)维空间中，![](assets/94c7724c-6e88-45ef-8716-5552846d1734.png)表示。为了确定向量之间的距离，我们可以使用任何度量；尤其是欧几里得距离。非线性 Sammon 映射的目标是搜索向量的选择，![](assets/2d7d5db0-c5ad-47ff-a45f-42eda67adf59.png)，以便最小化误差函数，![](assets/7bb434a8-837d-4321-9ef0-c353bca1d2b6.png)，其由以下公式定义:

![](assets/ea52954c-75ff-44b3-8118-6ba71f229d08.png)

为了使误差函数最小化，![](assets/d80f3882-ca74-4da5-80b2-77befea35b53.png)，Sammon 使用了牛顿最小化方法，可以简化如下:

![](assets/6d978124-f8c5-461b-80da-dc6ac93cb033.png)
...
![](assets/ab62c8b6-ca3f-4a5e-8156-f434b1335594.png)

该公式显示了点![](assets/dc984674-3d52-4ba6-b6f0-b7e3a7e8436f.png)与点![](assets/7ccd0343-4b11-48e1-a803-d562311ae898.png)的接近程度，高斯分布在![](assets/eae8ffe9-d45e-45e0-a47a-2b8e984811e7.png)周围，给定偏差为![](assets/3f076f20-96d4-47b1-ac8e-7dd99989e7d5.png)。![](assets/fa0dba0e-ac50-4d27-a5c5-ba0d24220151.png)每个点都不一样。选择它是为了使密度较高的区域中的点比其他区域中的点具有更小的方差。

让我们将(![](assets/0e85d6b5-b876-4696-8736-bc641a876bfc.png)、![](assets/1b4e9839-accc-4543-aa17-f15fb127e4fb.png))对的二维或三维映射表示为(![](assets/3e9a6ac2-9245-4f9a-b05d-8049f35bb9e1.png)、![](assets/a8c9da42-2f40-4cc6-8d01-c05b9e26f2ae.png))对。有必要使用相同的公式来估计条件概率。标准偏差为 <sub>![](assets/0add2e2d-b0c7-415b-acca-10e30b20819a.png)</sub> :

![](assets/eb8b543d-a201-4b0b-b643-719eb4c8ad3a.png)

如果映射点![](assets/9c6b9be0-f08a-45ba-98dc-26e19b7a99e6.png)和![](assets/4fcc38b5-b8bb-4ee9-919a-d402feaa7813.png)正确模拟了更高维的原始点![](assets/acef80ff-aa3c-4ced-a1ca-db95c6e08276.png)和![](assets/bfc23a36-9998-49db-af03-bfa8e7da390d.png)之间的相似性，那么对应的条件概率![](assets/fb4cb156-e617-4bfa-8dbf-8fb93529ecbb.png)和![](assets/60639bba-e022-44ab-834e-183b948e192b.png)将是等价的。作为对![](assets/8a225324-66ac-45b1-abf0-fb1c9bdb1319.png)如何反映![](assets/085f9feb-4421-467f-8405-e329ded6a4db.png)的质量的一个明显的评估，使用了散度或库尔巴克-莱布勒距离。SNE 使用梯度下降最小化所有映射点的这些距离的总和。以下公式确定了此方法的损失函数:

![](assets/cd903631-9755-4cca-b78b-682914c55818.png)

它具有以下梯度:

![](assets/b22ac7bd-1144-43f6-a0b5-a3e321cbbb72.png)

这个问题的作者为优化过程提出了以下物理类比。让我们假设弹簧连接了所有的映射点。弹簧连接点![](assets/ec0ebf56-0eec-4821-abc1-e34cd12d5faf.png)和![](assets/361626ad-9dbd-4121-8821-7ee3443bb185.png)的刚度取决于多维空间中的两点和映射空间中的两点的相似性之间的差异。在这个类比中，梯度是作用在映射空间中一个点上的合力。如果我们放开这个系统，一段时间后，它会产生平衡，这就是期望的分布。在算法上，它会在考虑以下时刻的同时寻找平衡:

![](assets/c92c2aaa-438c-4bde-b356-9451e67654b5.png)

这里，![](assets/2fdf9d66-48ad-4639-8a01-f7c5a5161d54.png)是学习率，![](assets/b4b2b8cc-e9eb-44f1-a6bc-27dfd550ccdf.png)是惯性系数。经典 SNE 也允许我们得到好的结果，但是在优化损失函数和拥挤问题时会有困难。总的来说，t-SNE 没有解决这些问题，但它让这些问题变得更容易管理。
...
![](assets/057afe32-fda3-4b16-a304-91e1c2197fd6.png)

现在，我们已经了解了可用于降维的线性和非线性方法，并详细探索了每种方法的组成部分，我们可以借助一些实际示例来增强降维的实现。

# 用各种с++库理解降维算法

让我们看看如何在实践中使用降维算法。所有这些示例都使用同一个数据集，该数据集包含四个正态分布的 2D 点集，这些点集已通过瑞士滚动映射![](assets/b02b0c5a-9b30-47e1-aa1b-3cf49c82fe0c.png)转换为三维空间。下图显示了此映射的结果。您可以在[http://people . cs . uchicago . edu/~ dinoj/manifold/Swiss roll . html](http://people.cs.uchicago.edu/~dinoj/manifold/swissroll.html)找到原始数据集和映射详情:

![](assets/abc91e95-c629-4a2b-9f0e-b0b99de76365.png)
...
![](assets/8486c110-d3eb-404e-9a62-0db156fdaec2.png)
...
![](assets/3d640a65-6823-4976-86fe-1f0420a3a4fa.png)

其原始灰度大小为 262，144 字节。在我们仅使用 10 个主成分执行 PCA 压缩后，其大小变为 45，760 字节。我们可以在下图中看到结果:

![](assets/b2bfacd6-57d0-46ab-a156-1b8745fc365d.png)
...
![](assets/f52e2ea1-98c2-499b-80b4-f6f43b1ec6fc.png)
...
![](assets/e9a8c862-8e1f-425f-8a09-6a7225009915.png)
...
![](assets/7fb4c749-1865-4fe3-99d3-701dd363d7a4.png)
...
![](assets/68ee4cce-a446-4870-910b-2b46ee82fd4f.png)
...
![](assets/29ac8ab8-c81f-4d7e-8f5e-f6b7cebc18ac.png)
...
![](assets/e17b7f92-8c03-4e71-b5f7-9cc88ce2305a.png)
...
![](assets/4d4b755f-cd71-4907-a1b9-a5731e7183ae.png)
...
![](assets/c68de7cb-97dd-4099-ae63-ecd13a747c5c.png)
...
![](assets/146548db-dc90-4428-bd58-73c92800d2dd.png)
...
![](assets/f05913a9-46f5-492f-a788-c70cde825d5e.png)
...
![](assets/696c5524-abf7-480e-9f1b-74bb294a8db7.png)
