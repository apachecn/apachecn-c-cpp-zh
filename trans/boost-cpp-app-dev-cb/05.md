...
正如我们从本章的第一个食谱中所知道的，如果传递到线程中的函数没有捕获到异常，并且该异常离开了函数边界，应用程序就会终止。`boost::thread_interrupted`是那条规则的唯一例外；它可能会离开函数边界，并且不会`std::terminate()`应用程序。
