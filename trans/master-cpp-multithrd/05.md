...
从 2011 年 C++标准修订版开始，多线程 API 正式成为 C++ **标准模板库** ( **STL** )的一部分。这意味着任何新的 C++应用程序都可以使用线程、线程原语和同步机制，而不需要安装第三方库，也不需要依赖操作系统的 API。
...
在[第 3 章](03.html)、 *C++多线程 API*中，我们查看了开发多线程 C++应用程序时可用的各种 API。在[第 4 章](04.html)、*线程同步和通信*中，我们使用本机 C++线程应用编程接口实现了一个多线程调度器应用程序。
 ...
 **一般来说，当 C++11 不支持时，或者当这些额外的`Boost.Thread`特性是一个人的应用程序的要求，并且不容易添加时，`Boost.Thread`应该作为一个后备。由于`Boost.Thread`建立在可用(本机)线程支持的基础上，与 C++11 STL 实现相比，它也可能增加开销。
...
前面这段代码将启动线程，然后立即终止应用程序，因为我们没有等待新线程完成执行。
...
这是一个高度专门化的功能，有特殊的用例。在没有首先验证它对应用程序性能的影响之前，不应该使用它。
...
如果您尝试按原样运行前面的代码，您会注意到来自两个线程的文本输出将被混合在一起，而不是一个接一个地输出。原因是标准输出(无论是 C 还是 C++风格)不是线程安全的。虽然应用程序不会崩溃，但输出将是一片混乱。
...
在下一章中，我们将了解多线程代码实现中不可避免的下一步:调试和验证生成的应用程序。**